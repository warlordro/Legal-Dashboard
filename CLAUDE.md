...
- `npm test --workspace=backend` â€” vitest backend (789 teste dupa v2.13.0: 751 baseline v2.12.1 + 17 in `services/email/dailyReportTemplate.test.ts` + 12 in `services/email/dailyReportScheduler.test.ts` + 7 in `routes/alerts.test.ts` "POST /export" + 2 in `routes/me.test.ts` "daily flag")
- `cd frontend && npm test -- --run` â€” vitest frontend (81 teste dupa v2.13.0: 73 baseline v2.12.1 + 3 in `lib/alertsApi.test.ts` "exportAlerts" + 3 in `lib/export-alerts.test.ts` + 2 in `components/EmailSettingsPanel.test.ts` "daily flag")
