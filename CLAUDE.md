...
## Versiune Curenta
**v2.7.0** - 2 Mai 2026 (PR-A din sprint-ul Dashboard redesign. **Backend:** endpoint nou `/api/v1/dashboard/summary` (owner-scoped, withMaintenanceRead) care agrega 4 blocuri intr-o singura cerere — joburi active cu byKind dosar_soap/name_soap, alerte unseen+last24h, rulari ok/error/timeout/total cu `aborted` foldat in error si still-running excluse, AI cost+calls+tokens 24h. **Frontend:** `KpiStrip` (4 carduri responsive) + `QuickActions` (6 butoane) inserate deasupra `LastDosareCard` in `pages/Dashboard.tsx`; polling 30s cu `AbortController`; al saselea button (Export raport) ramane disabled cu tooltip "Disponibil in v2.9.0 (PR-C)". `dashboardApi.summary()` reuseste `unwrapMonitoring`/`MonitoringApiError` din `lib/api.ts`. **Tests:** 7 teste noi in `routes/dashboard.test.ts` (envelope, empty state, jobs.byKind, alerts windowing, runs bucketing+still-running, AI aggregation, owner isolation cu 2 tenants); 553/553 verzi (era 546 baseline). **Coordonare:** Codex livreaza `feat/pr9-auth-pluggable` in paralel; PR-A pe `feat/dashboard-redesign` fara overlap pe fisiere.)
...
- 🚧 **PR-9 v2.7.x** - auth pluggable seam (branch `feat/pr9-auth-pluggable`): desktop noop + web JWT validation, fara SSO/cutover. Audit pack 2026-05-02 -> fix B1-B4 + 15 teste P0/P1 + docs sync.
...
- `npm test --workspace=backend` â€” vitest backend (553 teste, baseline din v2.7.0; +7 in PR-A peste 546 din v2.6.4)
