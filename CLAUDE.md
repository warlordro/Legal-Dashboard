...
- `npm test --workspace=backend` â€” vitest backend (728 teste dupa v2.11.0: 721 baseline v2.10.6 + 7 noi in `rnpm.contract.test.ts` — 3 pentru web-mode 501 gate pe `/search`/`/bulk`/`/captcha/balance` + 4 pentru admin guard cu `updateUserRole("local","user")` in `beforeEach`)
