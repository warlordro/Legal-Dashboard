...
## Versiune Curenta
**v2.6.6** - 1 Mai 2026 (UX polish Monitorizare — name_soap parity: buton `Dosare` + target bold pe randurile `name_soap` cu `onOpenName` propagat ca `pendingSearch numeParte`, label TIP rebrand "Subiect" → "Nume" pentru consecventa cu formularul si template-ul XLSX, swap coloane "Ultima rulare" inaintea "Urmatoarea verif." pentru lectura naturala fapte→predictie)
