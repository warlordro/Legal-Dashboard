...
## Versiune Curenta
**v2.6.8** - 1 Mai 2026 (Review-driven hardening peste v2.6.7. **Frontend:** cardul "Adaugare bulk din fisier" foloseste `<CardHeader role="button" tabIndex={0}>` cu `onKeyDown` Enter/Space (HTML invalid eliminat — `<button>` continea `<div>`+`<h3>`); `CADENCE_COL_LETTER` derivat din `HEADERS.indexOf("cadence_sec")` prin `colIndexToLetter` helper (reordonarea coloanelor nu mai poate desincroniza silent dropdown-ul OOXML); `parseBulkFile` raporteaza eroare clara "Header lipsa..." in `invalid[]` cand `findHeaderRow` esueaza (era silent return). **Docs:** `SESSION-HANDOFF.md` — claim-ul stale despre `xlsx@0.18.5` rescris (post-v2.6.4 nu mai e pe parser path).)
