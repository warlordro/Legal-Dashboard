...
- [ ] **DOC2 — README troubleshooting `dist:server`** — flow nou multi-stage Docker + bare-metal start (server-release ZIP nu mai contine node_modules; operatorul trebuie sa stie ca CMD `node dist-backend/index.cjs` necesita better-sqlite3 native binding). **Fisier:** [README.md](README.md) + comentariu in `start.sh`/`start.bat` din [scripts/build-server.js](scripts/build-server.js). **Effort:** ~30 min.
...
- [ ] **F10-H1 — `NODE_ENV=development` in `backend/.env.example`** — copiat 1:1 in deploy ZIP din `scripts/build-server.js` L30. Operator care deploy-eaza fara override → backend porneste in dev mode → CORS dev origins active + alte branch-uri dev-only deschise. Fix: scoate linia `NODE_ENV` din `.env.example` complet (sau seteaza `production`); CMD-ul Docker deja seteaza `ENV NODE_ENV=production`. Detectat de release-readiness-reviewer + repo-security-auditor. **Fisier:** [backend/.env.example](backend/.env.example). **Effort:** ~5 min.
- [ ] **F10-H2 — AbortSignal nu propaga prin SOAP layer** — [backend/src/services/batch-dosare.ts](backend/src/services/batch-dosare.ts) accepta `AbortSignal` si verifica intre iteratii, dar `cautareDosare` din [backend/src/soap.ts](backend/src/soap.ts) L60-71 are propriul `AbortSignal.timeout(45000)` intern fara optiune de externals signal. SSE timeout din `/load-more` (60s) abort-uieste la nivel de batch, dar SOAP-ul in zbor inca consuma 45s pana la propriul timeout → request-ul „pare" cancelled la client dar resursele backend continua. Fix: schimba semnatura `cautareDosare(params, { signal? })` + `AbortSignal.any([externalSignal, AbortSignal.timeout(45000)])`. Detectat de backend-reliability-reviewer. **Fisier:** [backend/src/soap.ts](backend/src/soap.ts) L60-71 + [batch-dosare.ts](backend/src/services/batch-dosare.ts). **Effort:** ~1h.
- [ ] **F10-H3 — SIGTERM in mijlocul `db.backup()` corupe `today.db`** — [backend/src/db/backup.ts:162](backend/src/db/backup.ts) `db.backup(dest)` scrie incremental in fisierul destinatie. SIGTERM la mijloc = fisier zero-or-partial la `legal-dashboard.YYYY-MM-DD.db`, care la urmatorul boot e considerat „backup valid" (mtime fresh, listBackupsWithMeta il vede). Restore din el = corruption silentioasa. Fix: scrie la `${dest}.tmp` apoi `rename` atomic ca la restoreFromBackup; pe SIGTERM cleanup `.tmp` daca exista. Cross-ref cu R1 (Faza 9) pentru fsync. Detectat de debug-investigator + backend-reliability-reviewer. **Fisier:** [backend/src/db/backup.ts:162](backend/src/db/backup.ts). **Effort:** ~30 min.
- [ ] **F10-H4 — Restore fara audit log** — [backend/src/db/backup.ts](backend/src/db/backup.ts) `restoreFromBackup` returneaza `{ preRestoreName }` la handler dar nu persista nicaieri „cine a restaurat ce, cand, din ce snapshot". Pentru compliance multi-tenant (Faza 5), trebuie linie persistenta in `audit_log` (cand exista). Pentru azi: console log structurat cu `{ action: "restore", source: name, preRestore: preRestoreName, ts }`. Detectat de audit-trail-reviewer + release-readiness-reviewer. **Fisier:** [backend/src/db/backup.ts](backend/src/db/backup.ts) `restoreFromBackup`. **Effort:** ~15 min (console) / ~2h (audit_log table).

### Medium priority (Faza 9 cleanup sau pre-feedback de la prima firma externa)

- [ ] **F10-M1 — Dockerfile fara lockfile** — `COPY backend/package.json ./` + `npm install` (L20) NU `npm ci` cu `package-lock.json`. Build-ul nu e reproducibil — orice patch-bump in registry intre ieri si azi schimba versiunea native. Fix: genereaza `backend/package-lock.json` (sau folder root cu `npm install --workspaces`), copy la imagine, `npm ci --omit=dev --build-from-source`. Detectat de release-readiness-reviewer + repo-security-auditor + dependency-style. **Fisier:** [Dockerfile](Dockerfile) L19-20. **Effort:** ~1h (genereaza + verify build).
- [ ] **F10-M2 — `HEALTHCHECK` fara `--start-period`** — [Dockerfile:50](Dockerfile) `HEALTHCHECK --interval=30s --timeout=10s --retries=3` fara `--start-period`. Pe DB ~100MB, prewarm + descriere migration ia 30-90s; primele healthchecks fail-uiesc pe `/health 503` si docker swarm/k8s marcheaza container-ul `unhealthy` → restart loop. Fix: `--start-period=120s`. **Fisier:** [Dockerfile:50](Dockerfile). **Effort:** ~5 min.
- [ ] **F10-M3 — Version bump v2.0.6 → v2.0.7 + CHANGELOG entry** — sprint-ul A1+DR1+DR3+C1-C4+I2+B1-B3 nu are bump in `package.json` + CHANGELOG.md. Fix: bump version + adauga sectiune CHANGELOG cu summary security-first sprint. **Fisiere:** [package.json](package.json) + [CHANGELOG.md](CHANGELOG.md). **Effort:** ~30 min.
- [ ] **F10-M4 — `fs.existsSync` regression in `restoreFromBackup`** — [backup.ts:120](backend/src/db/backup.ts) `if (fs.existsSync(dbPath))` e sync I/O in path async. Restul fisierului foloseste `fsPromises`. Inconsistent + sync IO blocheaza event loop. Fix: `await fsPromises.access(dbPath).then(() => true).catch(() => false)`. Detectat de deep-code-reviewer. **Fisier:** [backend/src/db/backup.ts:120](backend/src/db/backup.ts). **Effort:** ~10 min.
- [ ] **F10-M5 — WAL/SHM unlink ordering pre-rename** — [backup.ts:148-150](backend/src/db/backup.ts) sterge `-wal`/`-shm` DUPA `rename(tmpPath, dbPath)`. Pe POSIX cu hard-links + better-sqlite3 fast-open, exista o fereastra de race in care alt thread/handle deschide noua DB cu WAL stale. Fix: unlink `-wal`/`-shm` INAINTE de rename (sau folosesti `BEGIN IMMEDIATE; ROLLBACK` pe handle nou pentru checkpoint). Detectat de backend-reliability-reviewer. **Fisier:** [backend/src/db/backup.ts:148-150](backend/src/db/backup.ts). **Effort:** ~15 min.
- [ ] **F10-M6 — AI request logging** — [backend/src/routes/ai.ts](backend/src/routes/ai.ts) handler-urile nu log-eaza `{ provider, model, inputTokens?, outputTokens?, latencyMs, status }`. Pentru ops + cost-tracking + debugging spike-uri, e necesar minim un log structurat. Detectat de audit-trail-reviewer + release-readiness-reviewer. **Fisier:** [backend/src/routes/ai.ts](backend/src/routes/ai.ts). **Effort:** ~30 min.
- [ ] **F10-M7 — Docker CI workflow** — `.github/workflows/build-mac.yml` exista (commit `4f58b84`), dar nu e verificat ca Dockerfile-ul ridica. Fix: workflow nou `docker-build.yml` care `docker build .` + `docker run --rm legal-dashboard:test sh -c "node -e 'console.log(process.version)'"` ca smoke test. Cross-ref CI1 din Faza 8. Detectat de release-readiness-reviewer. **Fisier:** `.github/workflows/docker-build.yml`. **Effort:** ~1h.
- [ ] **F10-M8 — Test coverage pe backup atomicity edge cases** — `backup.test.ts` testeaza happy-path atomic restore + path traversal + missing. Lipsa: test pentru SIGTERM mid-`db.backup()` (simulate cu kill -9 wrapper), test pentru retention pool starvation reciproca, test pentru pre-migration retention bypass (F10-C3). Cross-ref T5 din Faza 8. Detectat de test-architect. **Fisier:** [backend/src/db/backup.test.ts](backend/src/db/backup.test.ts). **Effort:** ~2h.
