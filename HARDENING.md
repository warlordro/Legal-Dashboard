...
> Status update 2026-07-24: scan Claude Security (multi-agent, effort medium, tot repo-ul, rev `c5dd9697`) a produs **12 findings confirmate — 8 MEDIUM + 4 LOW, zero HIGH/CRITICAL**, deschise ca **Faza 12** mai jos. Prioritate 1 pentru web deploy: F12-F3 (cota RNPM ocolibila), F12-F5 (cheia 2Captcha in raspunsul de eroare), F12-F8 (gate admin PAT doar in renderer) — **toate trei LIVRATE 2026-07-26** (`967c853`, `0eaf679`, `65c6eed`). Cele 9 ramase sunt deschise. Raport complet: [audit/AUDIT-CLAUDE-SECURITY-SCAN-v2.43.2-2026-07-24.md](audit/AUDIT-CLAUDE-SECURITY-SCAN-v2.43.2-2026-07-24.md).

> Status update 2026-05-20: v2.34.0 inchide 4 P0 + 8 P1 din auditul intern post-v2.33.0 (`audit/AUDIT-FINAL-FULL-PROJECT-v2.33.0-2026-05-19.md`). Zonele livrate: GET/device-code dezactivat (POST-only) + RL `(ip, ua)`, admin guards intarite pe tenant-keys, SOAP retry budget, captcha balance per-tenant TTL, owner-scoped tenant key guard, body-key warning hardening, per-user captcha quota (count-based + rolling 24h/7d/30d + intent-recording), CI fixtures via `openssl rand`, offsite backup hook env-configurabil + RUNBOOK.md (12 sectiuni). Sentry SDK amanat la v2.35.0. Desktop neimpactat — toate schimbarile gateuite web-mode.

> Status update 2026-05-19: v2.33.0 inchide pachetul Security Hardening pentru CRITICAL-1 + 5 HIGH + 11 MEDIUM + 3 LOW din auditul `FIX-PLAN-v2.33.0`. Zonele livrate: quota/budget reservations, deployment topology, validation/external I/O si audit trail fara plaintext pentru secrete.
...
## Faza 12 — Scan Claude Security 2026-07-24 (rev `c5dd9697`, branch `feat/v2.43.0-rnpm-split`)

**Sursa:** scan multi-agent pe tot repository-ul la effort `medium`, focus attack-surface. 10 componente, 37 celule de research, 29 candidati → 25 dupa dedup → **12 confirmate** de un panel adversarial cu 3 lentile (reachability / impact / defenses), majoritate 2-din-3. Raportul complet cu fixurile exacte, scenariile de exploatare si preconditiile fiecarui finding: [audit/AUDIT-CLAUDE-SECURITY-SCAN-v2.43.2-2026-07-24.md](audit/AUDIT-CLAUDE-SECURITY-SCAN-v2.43.2-2026-07-24.md). Artefactele brute ale run-ului (`.jsonl` + stamp-ul de revizie) raman local in `CLAUDE-SECURITY-20260724-195947/`, director gitignorat.

**Zero HIGH, zero CRITICAL.** Nu s-a gasit RCE, SQL injection, bypass de autentificare in web sau escaladare de rol intre tenanti. Nimic nu a fost executat — findings-urile vin din citirea codului, deci confidence e plafonat la medium acolo unde exploatabilitatea depinde de comportament de runtime.

**Numerotare:** `F12-Fn` (ids-urile `Fn` din raportul original, prefixate ca sa nu se ciocneasca cu Faza 11).

### Blockers reale pentru web deploy (prioritate 1) — **TOATE TREI LIVRATE 2026-07-26**

Commituri: `967c853` (F12-F8), `0eaf679` (F12-F5), `65c6eed` (F12-F3), plus `706b9ae` pentru CodeRabbit 1.2 care sta in acelasi bloc de admitere. Plan si trasabilitatea review-ului adversarial: [docs/superpowers/plans/2026-07-26-f12-web-blockers.md](docs/superpowers/plans/2026-07-26-f12-web-blockers.md). Teste backend 2078 -> 2087. Celelalte 9 findings de mai jos raman deschise.

- [x] **F12-F3 — cota de stocare RNPM ocolibila cu orice `gcode` nenul** — **REZOLVAT `65c6eed`** (2026-07-26): scutirea pe `gcode` eliminata in ambele puncte; decizie user contra variantei cu `searchId` persistat. Cele doua teste care codificau scutirea (`rnpmStorageRecheck.test.ts:96`, `rnpmStorageLimit.routes.test.ts:166`) inversate. Original: (MEDIUM, panel 3/3) — [backend/src/services/rnpmSearchService.ts:371](backend/src/services/rnpmSearchService.ts) + [backend/src/routes/rnpm.ts:245](backend/src/routes/rnpm.ts). Un camp din body-ul clientului decide singur daca ruleaza `assertRnpmStorageWithinLimit`; un `"gcode":"x"` inventat sare peste verificare si la admitere si in bucla de paginare. **Impact:** singura cota de disc per tenant din produs devine decorativa — un cont peste plafonul de 750 MB scrie avize la infinit, iar toate fisierele `rnpm/<stem>.db` stau pe acelasi volum cu monolitul si backupurile. **Fix:** ori check neconditionat, ori gcode-uri persistate server-side per `(ownerId, searchId)` cu buget de pagini. **Atentie:** testul `rnpmStorageRecheck.test.ts:96` codifica comportamentul actual si trebuie actualizat odata cu fixul. **Effort:** ~3h.
- [x] **F12-F5 — cheia 2Captcha a tenantului scapa in raspunsul 500** — **REZOLVAT `0eaf679`** (2026-07-26): redactare pe VALOARE la constructia erorii in `captchaSolver.ts`, deci acopera toate cele SASE sinkuri (500 pe /search, SSE bulk, doua cai in split + nested tier-2, 400 pe /captcha/balance), nu doar cel raportat. `balance2Captcha` facea rethrow brut — era drumul spre 400. Plus mesaj generic pe /search. Original: (MEDIUM, panel 3/3) — [backend/src/routes/rnpm.ts:370](backend/src/routes/rnpm.ts) (+ `console.error` la 369, + `captchaSolver.ts:83`). SDK-ul trimite cheia ca query param, node-fetch pune URL-ul complet in mesajul erorii de transport, handler-ul serializeaza `err.message` brut. **Impact:** orice user non-admin recupereaza secretul platit partajat pe tenant (pe care `/admin/keys` il expune doar ca `last4`) si goleste portofelul de captcha. **Fix:** scrub `key=`/`clientKey` + query string in `captchaSolver.ts`, plus mesaj generic catre client asa cum face deja `ai.ts:229-231`. **Effort:** ~1.5h.
- [x] **F12-F8 — gate-ul admin pe PAT-uri exista doar in renderer** — **REZOLVAT `967c853`** (2026-07-26): `requireRole("admin")` pe router, DUPA gate-ul de PAT ca sa se pastreze `PAT_CANNOT_MANAGE_TOKENS`. Decizie user: PAT-urile raman admin-only. Nota OpenAPI aliniata. Original: (MEDIUM, panel 2/3) — [frontend/src/components/ApiKeyDialog.tsx:133](frontend/src/components/ApiKeyDialog.tsx) + [frontend/src/pages/Settings.tsx:110](frontend/src/pages/Settings.tsx); router-ul [backend/src/routes/apiTokens.ts](backend/src/routes/apiTokens.ts) n-are `requireRole("admin")`. **Impact:** orice user non-admin isi emite din consola browserului un bearer token de lunga durata, optional fara expirare, care ocoleste TTL-ul sesiunii, SameSite=Strict si revocarea `jti` la logout. Ramane owner-scoped, deci fara escaladare. **Fix:** `requireRole("admin")` pe router sau la mount (`index.ts:361`). Daca PAT-urile self-service sunt de fapt intentionate, atunci se scot gate-urile din UI si se corecteaza contractul — cele doua nu pot ramane divergente. **Effort:** ~30 min + test.

### DoS pe procesul backend partajat (prioritate 2)

- [ ] **F12-F1 — zip bomb XLSX pe `/name-lists/preview`** (MEDIUM, panel 3/3) — [backend/src/services/nameListParser.ts:347](backend/src/services/nameListParser.ts). `workbook.xlsx.load()` decomprima integral fiecare intrare ZIP; limitele existente masoara doar dimensiunea comprimata (10 MB) si randurile de dupa parsare. Timeout-ul de 30s raspunde clientului dar nu anuleaza parsarea. **Impact:** ~10 MB comprimati devin GB alocati, OOM pe procesul partajat = downtime pentru toti. **Fix:** validare pe central directory-ul ZIP (suma dimensiunilor necomprimate + prag de ratio) inainte de exceljs, sau parsare intr-un `worker_threads` cu `resourceLimits`. **Effort:** ~4h.
- [ ] **F12-F2 — acumulare nelimitata de dosare in SSE load-more** (MEDIUM, panel 3/3) — [backend/src/services/batch-dosare.ts:94](backend/src/services/batch-dosare.ts). Pana la 500 apeluri SOAP x 1000 inregistrari, toate retinute in `allDosare` pe durata request-ului, desi array-ul e citit doar pentru `.length`. Endpoint-ul non-streaming are plafon 5000 + 413; cel de streaming n-are niciunul. **Impact:** sute de MB pana la GB fixati in heap dintr-un singur request, conexiune tinuta 900s. Bonus: `sseEvent` nu consulta `controller.desiredSize`, deci un client care nu citeste creste coada stream-ului nelimitat. **Fix:** contor + set de dedup in loc de obiectele intregi, plafon analog cu `MAX_DOSARE_RESPONSE`, backpressure pe stream. **Effort:** ~3h.

### Dependinta cu CVE-uri deschise (prioritate 2)

- [ ] **F12-F7 — `xlsx-js-style@1.2.0` este SheetJS 0.18.5** (MEDIUM, panel 2/3) — [frontend/src/lib/monitoringBulkTemplate.ts:322](frontend/src/lib/monitoringBulkTemplate.ts), in `parseBulkFile`. Bundle-ul instalat se auto-raporteaza `version="0.18.5"` si n-are niciun guard `__proto__`: CVE-2023-30533 (prototype pollution) + CVE-2024-22363 (ReDoS) aplica. Plafonul de 10 MB si limita de randuri ruleaza inainte respectiv dupa parsare, niciodata in interiorul parserului. **Impact:** un fisier trimis victimei si importat prin bulk import polueaza `Object.prototype` in realm-ul rendererului sau ingheata thread-ul UI. In Electron rendererul e sandboxat (corupere + DoS, nu RCE); in web ruleaza in tabul fiecarui user care importa un fisier primit. **Fix:** parsare cu exceljs (deja dependinta backend) sau SheetJS >= 0.20.2, sau Web Worker de unica folosinta. **Effort:** ~1 zi.
- [ ] **F12-F7b — corectare afirmatii false despre CVE-urile xlsx** (documentatie) — [frontend/src/lib/monitoringBulkTemplate.ts:29-32](frontend/src/lib/monitoringBulkTemplate.ts) si [SECURITY.md:283](SECURITY.md) sustin ca migrarea la `xlsx-js-style` a inchis aceste CVE-uri. Este fals: `xlsx-js-style` 1.2.0 e un fork al versiunii afectate. **De corectat indiferent daca F12-F7 se implementeaza acum** — o afirmatie falsa de securitate in documentatie e mai periculoasa decat lipsa ei. Vezi si nota de la finalul acestei faze despre asumptiile invechite din Faza 2. **Effort:** ~30 min.

### Desktop-only (prioritate 3 — nu blocheaza web deploy)

- [ ] **F12-F4 — DNS rebinding: zero validare de `Host` in mod desktop** (MEDIUM, panel 3/3) — [backend/src/middleware/requireDesktopHeaderGlobal.ts:22](backend/src/middleware/requireDesktopHeaderGlobal.ts). In desktop fiecare request e owner `local` promovat admin ([index.ts:555-565](backend/src/index.ts)); `originGuard` iese devreme pentru orice peer loopback iar headerul custom e trivial de setat. **Impact:** o pagina ostila al carei DNS se rebind-uieste la 127.0.0.1 devine same-origin: citeste tot setul de date si apeleaza rutele admin (RNPM `DELETE /saved/all`, `POST /compact`, backup management). GET-urile nu trec deloc prin acest middleware. **Fix:** allowlist de `Host` in fata router-ului cand bind-ul e loopback. **Effort:** ~2h.
- [ ] **F12-F6 — fereastra Electron incarca `localhost`, backendul asculta doar IPv4** (MEDIUM, panel 3/3) — [electron/main.js:409](electron/main.js). `[::1]:3002` ramane liber pentru orice proces local, iar singura verificare de identitate (`bootNonce`, [main.js:270](electron/main.js)) se face pe o conexiune separata, cu un nonce pe care orice proces local il poate citi de la `/health` si relua. **Impact:** un squatter local primeste preload-ul privilegiat, origin-ul `http://localhost:3002` cu cifrotextul safeStorage din localStorage si `desktopApi.decryptKeys` — cere aplicatiei victimei sa-i decripteze cheile Anthropic / OpenAI / Google / OpenRouter / 2Captcha / CapSolver. **Fix:** incarca si health-check-uieste literal `http://127.0.0.1:${PORT}`, restrange `isAllowedNavUrl` la acelasi host, si nu trata un ecou de la `/health` neautentificat ca dovada de identitate (port efemer ales in-process, sau nonce ca header validat de backendul real). **Effort:** ~2h.

### Igiena (prioritate 4)

- [ ] **F12-F9 — CRLF injection neautentificata in logul de auth** (LOW, panel 3/3, confidence high) — [backend/src/middleware/owner.ts:53](backend/src/middleware/owner.ts), caz identic la [backend/src/routes/rnpmGuards.ts:97](backend/src/routes/rnpmGuards.ts). `getPath` din Hono trece prin `decodeURI`, iar `%0A`/`%0D` nu sunt in setul rezervat. **Impact:** linii fabricate in stream-ul de loguri care e principala dovada pentru abuz de autentificare; `audit_log` nu e afectat (scriere parametrizata). **Fix:** `JSON.stringify` pe path sau forma structurata folosita deja de access logger (`index.ts:106`). **Effort:** ~1h (ambele locuri).
- [ ] **F12-F10 — oracol cross-owner de existenta a joburilor de monitorizare** (LOW, panel 3/3, confidence high) — [backend/src/routes/monitoring.ts:303](backend/src/routes/monitoring.ts) si [monitoring.ts:397-404](backend/src/routes/monitoring.ts). Sonda in Map-ul `inflight` (cheie doar pe jobId, fara owner) ruleaza inaintea `getJobById(ownerId, id)`. **Impact:** un tenant distinge 409 „jobul altcuiva ruleaza acum" de 404 „nu exista / nu e al meu"; cu `bulk-delete` sondeaza 100 de id-uri secventiale odata si isi construieste un inventar cross-tenant plus cadenta de rulare — exact ce comentariul din cod de la 407-408 isi propune sa previna. **Fix:** rezolva ownership-ul inainte de sonda, in ambele handlere; alternativ cheie `(ownerId, jobId)` in scheduler. **Effort:** ~1h.
- [ ] **F12-F11 — newline in `institutie` ajunge nescapat in log** (LOW, panel 3/3) — [backend/src/routes/termene.ts:154](backend/src/routes/termene.ts); `validateParams` ([util/validation.ts:42](backend/src/util/validation.ts)) omite deliberat `\n`, `\r`, `\t` din clasa respinsa. Aceeasi categorie ca F12-F9, suprafata mai ingusta (cere 2+ valori `institutie` si un `soap:Fault` pe cea injectata). **Fix:** extinde clasa in `validateParams` si/sau log structurat in loc de interpolare. **Effort:** ~1h.
- [ ] **F12-F12 — SMTP fara `requireTLS`** (LOW, panel 2/3) — [backend/src/services/email/mailer.ts:59](backend/src/services/email/mailer.ts). `secure` derivat din `port === 465`, `requireTLS` niciodata setat; pe portul 587 conexiunea urca la TLS doar daca serverul anunta STARTTLS. **Impact:** un atacator on-path intre container si releu sterge anuntul, nodemailer face AUTH in clar si preda `SMTP_USER`/`SMTP_PASS`, plus continutul alertelor (numere de dosar, nume de parti). **Fix:** `requireTLS: true` cand `config.secure === false`. **Effort:** ~15 min.

### Asumptii invechite de corectat in acest fisier

Doua afirmatii din backlog sunt contrazise direct de F12-F7 si trebuie tratate cand se atinge faza respectiva:

Faza 2, itemul „Bump dompurify + jspdf" spune **„NU migrare xlsx → exceljs (xlsx e folosit doar pentru export/write, nu parse — CVE-urile nu aplica)"**. Este invechit: `parseBulkFile` din `monitoringBulkTemplate.ts` parseaza fisiere netrusted urcate de user. Acelasi lucru pentru bulletul „Migrare xlsx → exceljs" din sectiunea „Items ce NU sunt aici".

**Total Faza 12: ~2.5-3 zile** (prioritatea 1 singura: ~5h; excluzand F12-F7 care e ~1 zi de migrare parser).
