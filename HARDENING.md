...
- [ ] **DR1 — `restoreFromBackup` non-atomic** — [backend/src/db/backup.ts:97-114](backend/src/db/backup.ts) face `closeDb()` apoi `fs.copyFile(backup, dbPath)` direct peste DB live. Crash/power-loss in mijlocul copy → DB corupta, WAL/SHM pointeaza la snapshot vechi. Fix: stage la `dbPath + ".restore.tmp"`, unlink `-wal`/`-shm` best-effort, `fs.rename` (atomic same-FS). **Effort:** ~1h.
...
- [ ] **DR3 — boot migration blocheaza request path** — [backend/src/index.ts:90-99](backend/src/index.ts) `serve()` binds portul inainte ca migratia sincrona din [schema.ts:267-299](backend/src/db/schema.ts) sa termine. Pe DB ~100MB, VACUUM sincron blocheaza 30-90s → Electron `STARTUP_TIMEOUT_MS=30000` ([electron/main.js:122](electron/main.js)) blow past → user vede „Backend nu raspunde". Fix: ruleaza `initSchema()` + descriere migration inainte de `serve()`; `/health` returneaza 503 pana cand gata. Complementar cu I4 (splash). **Effort:** ~1h.
...
- [ ] **A1 — escape closing pseudo-tags in `buildPrompt`/`buildJudgePrompt`** — [backend/src/services/ai.ts](backend/src/services/ai.ts) splice-uieste text user/LLM in `<dosar_data>...</dosar_data>`, `<analiza_n>...</analiza_n>` fara escape. User-controlled `descriere` cu `</dosar_data>` literal rupe granita → LLM trateaza restul ca instructiuni. Pentru judge: output-ul primului model e attack-surface pentru al doilea (indirect prompt injection clasic). Fix: `text.replaceAll("</dosar_data>", "<\\/dosar_data>")` (idem pt. toate tag-urile), sau foloseste UUID-delimited fence per request. Cap pe per-field length dupa escape. **Effort:** ~1h.
...
- [ ] **W4 — `.dockerignore` la repo root** — complement la C3 (Faza 7). Contextul `docker build` trimite tot worktree-ul (inclusiv `.env.local`, `node_modules`, `release/`, `server-release/`). Fix: `.dockerignore` cu `.env*`, `node_modules/**`, `release/`, `server-release/`, `.git`, `dist/`, `dist-*/`. Zero `--no-cache`. **Effort:** ~10 min.
