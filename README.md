...
Versiune curenta: **v2.40.0**. Vezi [CHANGELOG.md](CHANGELOG.md) pentru istoric,
...
Ultimul release **v2.40.0** - API programatic doar-citire prin Personal Access Tokens (Piesa A din planul API + MCP, doar web mode): dosare + termene (PortalJust), ICCJ si RNPM accesibile din scripturi/integrari externe cu tokenuri opace `ld_pat_...` (hash SHA-256 in DB, secret afisat o singura data). Gate default-deny pe tuple `(metoda, path, scope)`, management session-only (un PAT nu poate administra tokenuri), HTTPS-only + no-store, rate-limit si plafon captcha per-token (rezervare atomica fail-closed), audit pe fiecare folosire + alerta email la IP nou, circuit breaker ICCJ ponderat pe clasa de caller. Self-description `GET /api/v1/openapi.json` (OpenAPI 3.1) + ghid consumator [API.md](API.md); UI Setari -> Acces API (doar web). Desktop: zero impact. Predecesor **v2.39.0** - Fix major pentru deploy-ul web: sesiunea se stabileste automat la incarcare (`useSessionBootstrap` + gate pe `AuthedApp`), se reinnoieste in fundal si se autorecupereaza la 401, deci utilizatorul ramane logat cat tine aplicatia deschisa, fara refresh manual.
