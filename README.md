...
Ultimul release **v2.43.0** - separare fizica a datelor RNPM per utilizator: fiecare cont primeste propriul fisier SQLite `rnpm/<stem>.db` (split one-time crash-safe la primul boot, cu backup pre-split verificat), backup si restaurare self-service strict pe datele proprii (cooldown, garduri de concurenta cautare/restore, snapshot pre-restore verificat), backup zilnic multi-target cu retentie pe pool-uri disjuncte si tab nou Setari > Backup (admin-only) pentru baza completa. Include autocompact dupa stergeri, limita de stocare RNPM per utilizator si plafon best-effort pentru backup-urile RNPM. Predecesor **v2.42.2** - corectii post-review pentru body limits, rate limiting si shutdown Electron. Predecesor **v2.42.1** - hardening web/desktop si tenant isolation. Predecesor **v2.42.0** - administrare utilizatori, cote AI, consum si audit in web mode.
