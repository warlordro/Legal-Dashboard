...
Versiune curenta: **v2.39.0**. Vezi [CHANGELOG.md](CHANGELOG.md) pentru istoric,
...
Ultimul release **v2.39.0** - Fix major pentru deploy-ul web: in `auth_mode=web` SPA-ul nu apela niciodata bridge-ul `POST /api/v1/auth/oauth2/sync`, deci cookie-ul de sesiune nu se minteia niciodata si fiecare apel `/api` intorcea 401 "Token de autentificare necesar" (cautare, `/me`, chei blocate desi login-ul Google reusea). Acum sesiunea se stabileste automat la incarcare (`useSessionBootstrap` + gate pe `AuthedApp`), se reinnoieste in fundal (`useSessionKeepAlive` la ~50min + wake pe `visibilitychange`/`online`) si se autorecupereaza la 401 (interceptor deduplicat + retry in `apiFetch`), deci utilizatorul ramane logat cat tine aplicatia deschisa, fara refresh manual. Desktop: zero impact. Predecesor **v2.38.0** - Refresh de modele AI (Claude Opus 4.6 -> 4.8, Gemini 3 Flash -> 3.5 Flash, nativ + OpenRouter), eliminarea stack-ului OpenRouter chinezesc (GLM/Kimi/Qwen; migratia 0036 coerce `chinese` -> `western`) si un val de hardening de securitate: revocare JWT cu `jwt_denylist` la logout (migratia 0038), cookie de sesiune SameSite Lax -> Strict, retragerea boot gate-ului redundant `LEGAL_DASHBOARD_ACK_NO_AUTH`.
