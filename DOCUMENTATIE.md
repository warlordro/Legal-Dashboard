...
- **Versiune curenta**: **v2.20.8** (12 Mai 2026, hardening operational ce inchide Batch 2 (Operator visibility) si Batch 4 (Scheduler & captcha reliability) din `FIXES-TODO.md`: (1) `source_partial` alert in spate de `MONITORING_PARTIAL_ALERTS_ENABLED=1` cand una sau mai multe institutii SOAP esueaza dar restul reusesc; (2) `/health` expune `emailConfigured: boolean` (derivat din `SMTP_HOST`); (3) `preMigrationBackup("schema-upgrade")` inainte de `process.exit(1)` in `fatalBoot`; (4) cleanup `-wal`/`-shm` pe auto-revert backup; (5) VACUUM splash blocking peste Baza locala RNPM (`role="alertdialog"`, `aria-busy`) ca DB-ul sa nu fie inchis midstream; (6) `.catch` handler pe `runOne` fire-and-forget in scheduler — elimina runs `running` stuck; (7) `AbortSignal.timeout(15_000)` pe `getBalance()` (2Captcha + CapSolver); (8) race-mode sleep signal-aware prin `Promise.race`; (9) retry exponential pe daily report email scheduler `[5/15/45]min`, audit `retry_exhausted` dupa 3 esecuri (state in-memory `Map<ownerId, retryState>`); (10) `setInterval(5min)` periodic sweep pe rate-limit middleware (in plus fata de threshold-based cleanup existent). Zero schimbari schema, zero schimbari contract API. **+10 teste backend (854/854) + 100/100 frontend**). Pentru istoric complet vezi [CHANGELOG.md](CHANGELOG.md) si in-app changelog (`/changelog`).
