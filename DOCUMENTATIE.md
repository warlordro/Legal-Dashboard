...
- **Versiune curenta**: **v2.28.2** (17 Mai 2026, bugfixes + UX tuning + ops watchdog peste audit refactor Tier 1+2 livrat ca grup de 13 commits: fix `[object Object]` la cautari RNPM (envelope v2.14.0 ne-dezambalat in 4 site-uri din `rnpmApi.ts` + reactivarea flow-ului Split rupt silent din v2.14.0), fix native AI mode (regresie v2.28.0 — `routing.mode === "native"` invinge auto-detect-ul pe env / saved key), 3-state ownership in `searchRepository` (`owned/foreign/missing`) care trateaza graceful searchId-ul cache-uit dupa "Sterge baza", event-loop watchdog Electron (`electron/event-loop-watchdog.js`, polleaza 5s cu `node:perf_hooks.monitorEventLoopDelay`, scrie `diagnostic/event-loop-lag.log` + `process.report.writeReport()`), plus header-ele AI strangite cu -33% inaltime + vertical-align pe rows /monitorizare + notite inline constrante la 12rem). Pentru istoric complet vezi [CHANGELOG.md](CHANGELOG.md) si in-app changelog (`/changelog`).
