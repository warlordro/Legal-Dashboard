...
- **Versiune curenta**: **v2.20.7** (11 Mai 2026, polish release dupa v2.20.6 — trei interventii narrow-scope independente: (1) export "toate avizele filtrate" in Baza locala RNPM (Excel/PDF), nu doar pagina vizibila; `rnpmGetAllSaved` aduce inregistrarile pe pagini de 200 prin `/saved`, apoi `rnpmExport` trimite in batch-uri de 500 IDs prin `/saved/export`, backend neatins; (2) sheet "Debitori" -> "Parti" in export (xlsx + PDF + stats line) — schimbarea reflecta ca sheet-ul contine entitati cu rol/calitate variata (Cesionar, Cedent, Debitor cedat, Garant) si nu literalmente debitori (verificat empiric: TELECREDIT IFN are 106 inregistrari ca Cesionar, una ca Cedent, zero in `rnpm_creditori`); DB schema (`rnpm_creditori` / `rnpm_debitori`) neatinsa, schimbarea e la presentation layer; (3) toggle in-memory in Setari -> Notificari sistem pentru popup-urile Windows/macOS legate de alerte — early return in `useAlertsStream.showDesktopNotification`, bulina cu numar de unread si pagina Alerts neafectate, preferinta nu persista (default ON la fiecare restart) si nu queue-uieste nimic (la reactivare nu vine flood); butonul Test in panou se dezactiveaza odata cu toggle-ul. Micro-fix UX: tabul Bulk RNPM ramane montat la schimbare de tab in RnpmSearch — o cautare Bulk in progres nu mai e anulata la peek pe "Search" sau "Saved". Backend neatins, 844/844 backend + 100/100 frontend.). Pentru istoric complet vezi [CHANGELOG.md](CHANGELOG.md) si in-app changelog (`/changelog`).
