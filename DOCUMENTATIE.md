...
- **Versiune curenta**: **v2.20.6** (10 Mai 2026, hygiene release — `.env.example` reconstruit la root cu toate cele ~25 env vars folosite in cod (grupate in 7 sectiuni: mod si bind, auth web mode, storage, monitoring, email SMTP, AI providers, RNPM kill switches; adnotate `REQUIRED-WEB | OPTIONAL`) ca sa inchida violarea CP-2 din root `CLAUDE.md`. Plus microfix envelope pe `requireRole.ts` (admin guard): cele 3 cai de denial 401/403 returneaza acum `fail(code, message, c)` cu `data: null` + `requestId`, schimbare aditiva (testele asertea pe `body.error.code/message`). Migrarea envelope pe celelalte rute legacy (rnpm/dosare/termene/ai) ramane EXPLICIT amanata pentru PR-6 — policy din `util/envelope.ts` + guard `rnpm.contract.test.ts` o cer one-shot odata cu `@hono/zod-openapi`, nu incrementala. Zero schimbari de contract HTTP pe rute non-admin, zero migration noua.). Pentru istoric complet vezi [CHANGELOG.md](CHANGELOG.md) si in-app changelog (`/changelog`).
