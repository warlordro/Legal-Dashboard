...
describe("auth jwt", () => {
  ...
  it("rejects tampered signatures", () => {
    ...
    expect(() =>
      verifyAuthToken(tampered, { secret: SECRET, nowSeconds: 50 }),
    ).toThrow(/signature/i);
  });
  ...
  it("rejects expired tokens", () => {
    ...
    expect(() =>
      verifyAuthToken(token, { secret: SECRET, nowSeconds: 101 }),
    ).toThrow(/expired/i);
  });
  ...
  it("rejects issuer and audience mismatches", () => {
    ...
    expect(() =>
      verifyAuthToken(token, { secret: SECRET, nowSeconds: 50, issuer: "other" }),
    ).toThrow(/issuer/i);
    expect(() =>
      verifyAuthToken(token, { secret: SECRET, nowSeconds: 50, audience: "other" }),
    ).toThrow(/audience/i);
  });

  it("rejects malformed tokens without three JWT parts", () => {
    expectTokenCode(
      () => verifyAuthToken("not-a-jwt", { secret: SECRET, nowSeconds: 50 }),
      "invalid_token",
    );
  });
  ...
  it("rejects alg=none tokens before signature validation", () => {
    ...
    expectTokenCode(
      () => verifyAuthToken(token, { secret: SECRET, nowSeconds: 50 }),
      "unsupported_alg",
    );
  });
  ...
  it("rejects payloads with missing or non-number exp", () => {
    ...
    expectTokenCode(
      () => verifyAuthToken(missingExp, { secret: SECRET, nowSeconds: 50 }),
      "token_expired",
    );
    expectTokenCode(
      () => verifyAuthToken(stringExp, { secret: SECRET, nowSeconds: 50 }),
      "token_expired",
    );
  });
