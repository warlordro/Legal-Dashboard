-- 0008_name_soap_alert_kinds.down.sql - restore the PR-4 alert-kind CHECK.
-- Rows using PR-5-only alert kinds cannot fit the old constraint, so down
-- migration fails loudly if such rows exist (CHECK constraint violation),
-- preserving the transaction and forcing an explicit operator decision.
...
DELETE FROM _schema_versions WHERE version = 8;
