-- 0008_name_soap_alert_kinds.up.sql - allow PR-5 name_soap diff alerts.
--
-- PR-5 name_soap emits stadiu/categorie/relevance transitions. 0003's CHECK
-- constraint listed only PR-4 dosar_soap alert kinds, so a valid runner would
-- fail on its first useful alert. SQLite cannot ALTER a CHECK constraint in
-- place; rebuild the table and preserve all existing rows.
