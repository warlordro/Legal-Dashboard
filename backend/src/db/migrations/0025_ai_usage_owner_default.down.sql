-- 0025 DOWN - drop the DEFAULT 'local' on owner_id, restore 0024 UP schema.
-- No data loss: schema and rows are identical except the column default.
-- Pre-rollback backup is mandatory.

CREATE TABLE ai_usage_v24 (
...
DROP TABLE ai_usage;
ALTER TABLE ai_usage_v24 RENAME TO ai_usage;
...
DELETE FROM _schema_versions WHERE version = 25;
