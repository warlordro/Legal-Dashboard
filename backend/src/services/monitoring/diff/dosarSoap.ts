...
export function computeFilterFingerprint(alertConfig: AlertConfig): string {
  const stadii = alertConfig.stadii
    ? Array.from(new Set(alertConfig.stadii.map(normalizeStadiu))).sort()
    : null;
  const categorii = alertConfig.categorii
    ? Array.from(new Set(alertConfig.categorii.map(normalizeStadiu))).sort()
    : null;
  return createHash("sha256")
    .update(canonicalJson({ stadii, categorii }), "utf8")
    .digest("hex");
}
...
function buildSnapshot(
  ...
  for (const s of dosar.sedinte) {
    ...
    sedinteWithSolution[buildSedintaKeyWithoutSolutie(input)] =
      (s.solutie ?? "").trim().length > 0;
  }
  return {
    sedintaKeys,
...
export function diffDosarSoap(input: DiffInput): DiffOutput {
  ...
  const filteredDosar =
    currentDosar && dosarPassesFilter(currentDosar, alertConfig) ? currentDosar : null;
  ...
  const candidateMissingSet = new Set(
    prevSnapshot.sedintaKeys.filter((k) => !currentKeys.has(k)),
  );
  ...
  // Pass 1: solutie_aparuta. A current "new" key that shares its
  ...
  // Pass 2: termen_changed pairing. For each remaining new sedinta, see if
  // exactly one prev "missing" sedinta shares (stadiu, complet) — that's a
  // reschedule. Multiple matches = ambiguous (fall back to termen_new) so we
  // never invent a pairing that didn't happen.
