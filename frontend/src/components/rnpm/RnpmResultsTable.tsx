...
import { Eye, Loader2, Download, ArrowUp, ArrowDown, ArrowUpDown, ChevronRight } from "lucide-react";
      ...
      {(totalPages > 1 || hasMore) && (
        <div className="flex flex-col items-center gap-2 border-t border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}>‹ Inapoi</Button>
            ...
            <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1}>Inainte ›</Button>
            <Button variant="outline" size="sm" onClick={() => setPage(totalPages - 1)} disabled={page === totalPages - 1}>»</Button>
            {hasMore && (
              <Button variant="outline" size="sm" disabled={loading} onClick={onNeedMore} className="ml-2">
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                Incarca mai multe ({loadedCount} din {result.total})
              </Button>
            )}
