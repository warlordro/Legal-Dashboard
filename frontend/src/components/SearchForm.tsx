...
export function SearchForm({ onSearch, onCategoriiChange, onStadiiChange, onInstitutiiChange, onDateChange, loading, showDateRange, showFilters = true, showLoadMore, loadingMore, onLoadMore, onStopLoadMore, loadMoreMessage, loadMoreProgress, loadMoreLabel = "dosare", loadMoreDone, loadMoreTotal, loadMoreWarnings, defaultParams, onReset }: SearchFormProps) {
  ...
  const toggleStadiu = (stadiu: string) => {
    setParams((p) => {
      ...
      const next = current.includes(stadiu)
        ? current.filter((s) => s !== stadiu)
        : [...current, stadiu];
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-1.5">
              <label htmlFor={ids.numarDosar} className="text-xs font-medium text-muted-foreground">Numar Dosar</label>
              <Input id={ids.numarDosar} placeholder="ex: 1234/2/2024" value={params.numarDosar ?? ""} onChange={set("numarDosar")} />
            </div>
            <div className="space-y-1.5">
              <label htmlFor={ids.numeParte} className="text-xs font-medium text-muted-foreground">Nume Parte</label>
              <Input id={ids.numeParte} placeholder="ex: Popescu Ion" value={params.numeParte ?? ""} onChange={set("numeParte")} />
            </div>
            <div className="space-y-1.5">
              <label htmlFor={ids.obiectDosar} className="text-xs font-medium text-muted-foreground">Obiect Dosar</label>
              <Input id={ids.obiectDosar} placeholder="ex: litigiu proprietate" value={params.obiectDosar ?? ""} onChange={set("obiectDosar")} />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <InstitutieSelect
              value={Array.isArray(params.institutie) ? params.institutie : params.institutie ? [params.institutie] : []}
              onChange={(val) => {
          ...
          {showDateRange && (
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <label htmlFor={ids.dataStart} className="text-xs font-medium text-muted-foreground">Data Start (optional)</label>
                <Input id={ids.dataStart} type="date" value={params.dataStart ?? ""} onChange={(e) => {
              ...
              <div className="space-y-1.5">
                <label htmlFor={ids.dataStop} className="text-xs font-medium text-muted-foreground">Data Stop (optional)</label>
                <Input id={ids.dataStop} type="date" value={params.dataStop ?? ""} onChange={(e) => {
                  const val = e.target.value || undefined;
          ...
          {loadingMore && loadMoreProgress && (
            <div className="space-y-1.5">
              <p className="text-xs font-medium text-foreground">
                Luna {loadMoreProgress.processed} din {loadMoreProgress.total} — {loadMoreProgress.found.toLocaleString("ro-RO")} {loadMoreLabel} noi gasite
              </p>
            ...
            <div className="space-y-1">
              ...
              {loadMoreWarnings && loadMoreWarnings.length > 0 && (
                <div className="text-xs text-amber-600 dark:text-amber-400">
                  {loadMoreWarnings.map((w, i) => <p key={i}>⚠ {w}</p>)}
                </div>
              )}
            </div>
          )}
