      ...
      <div className="rounded-lg border border-violet-200 dark:border-violet-800 bg-violet-50/30 dark:bg-violet-950/20">
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <ChevronDown
              className={`h-4 w-4 text-violet-500 transition-transform ${ai.collapsed.has(`ai-${dosar.numar}`) ? "rotate-180" : ""}`}
            />
          </div>
        </button>
        {ai.collapsed.has(`ai-${dosar.numar}`) && (
          <div className="px-4 pb-4 space-y-3">
              ...
              <div className="flex flex-col gap-1.5">
                {Object.entries(ai.providerGroups).map(([provider, models]) => {
                  ...
                  return (
                    ...
                    >
                      <span className="px-1.5 text-[10px] font-semibold text-muted-foreground uppercase w-14">
            ...
            >
              {ai.loading === dosar.numar ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}
            ...
            {ai.showKeyPrompt && !ai.hasAnyKey && (
              <p className="text-sm text-orange-600 dark:text-orange-400">
                ...
                Configureaza cel putin o cheie API din <strong>Setari API</strong> (meniul din stanga) pentru a utiliza
                aceasta functie.
              </p>
            )}
      ...
      <div className={`mt-3 rounded-lg border ${mc.border} ${mc.bg}`}>
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <ChevronDown
              className={`h-4 w-4 ${mc.chevron} transition-transform ${ai.collapsed.has(`multi-${dosar.numar}`) ? "rotate-180" : ""}`}
            />
          </div>
        </button>
              ...
              <div className={`flex items-center gap-0.5 rounded-lg border ${mc.selectBorder} bg-background p-0.5`}>
                ...
                {ai.availableModels.map((m) => (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      multi.setAnalysts((prev) => [m.key, prev[1]]);
                    }}
                ...
                {(() => {
                  const sel = ai.availableModels.find((m) => m.key === multi.analysts[0]);
              ...
              <div className={`flex items-center gap-0.5 rounded-lg border ${mc.selectBorder} bg-background p-0.5`}>
                ...
                {ai.availableModels.map((m) => (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      multi.setAnalysts((prev) => [prev[0], m.key]);
                    }}
                ...
                {(() => {
                  const sel = ai.availableModels.find((m) => m.key === multi.analysts[1]);
                  return sel ? <span className="px-1.5 text-[10px] text-muted-foreground">{sel.desc}</span> : null;
                })()}
              </div>
              {/* Judge */}
                          ...
                          <div className="prose prose-sm max-w-none text-xs leading-relaxed text-muted-foreground [&_strong]:font-semibold [&_strong]:text-foreground [&_p]:my-1 max-h-[400px] overflow-y-auto">
                            {a.text.split("\n").map((line, li) => {
                              if (
                                ...
                                return (
                                  <h3 key={li} className="text-xs font-semibold mt-2 mb-0.5">
                                    {line.replace(/^#{2,3}\s|^\*\*|\*\*$/g, "")}
                                  </h3>
                                );
                              if (line.startsWith("- ") || line.startsWith("* "))
                                return (
