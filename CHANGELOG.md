...
**2. Corpul cererii de cautare apare acum in specul tehnic al API-ului.** Pana acum `POST /api/rnpm/search` era listata in `GET /api/v1/openapi.json` fara niciun camp descris, deci intr-un vizualizator (Swagger UI, Postman, un generator de client) integratorul vedea ruta si atat — tot ce tine de forma cererii trebuia citit din `API.md`, ceea ce presupune ca cineva chiar deschide documentul. Acum ruta are schema completa a corpului: tipurile de inscriere ca lista de valori, filtrele pe rol cu scrierea exacta a cheilor, paginarea, si `includeDetails` ca bifa cu explicatie. Trei lucruri care erau invizibile in afara documentatiei si se ghiceau: operatorul dintre criterii (`type` e `"1"` pentru SI si `"2"` pentru SAU — un numar fara sens pana acum), faptul ca fiecare criteriu e un obiect cu valoare plus operator, si plafoanele reale de paginare. Un test verifica in plus ca toate referintele interne ale specului rezolva, fiindca o referinta rupta nu da eroare la generare: campul apare pur si simplu gol in vizualizator.

**3. Documentatia de integrare: cheile de filtrare pe rol.** `API.md` are o sectiune noua (§5b) cu contractul de mai sus si cu tabelul cheilor exacte pentru filtrarea dupa creditor si debitor. Motivul e o capcana reala: cheile sunt copiate dupa cele ale registrului RNPM, deci majusculele nu sunt uniforme — `creditorPJ.regCom` are `r` mic, `debitorPJ.RegCom` are `R` mare, iar `CreditorPF` incepe cu majuscula in timp ce `debitorPF` nu. O cheie scrisa gresit e **ignorata in tacere**: filtrul nu se aplica, raspunsul contine tot, si clientul nu primeste niciun avertisment. Semnul ca filtrul a functionat ramane scaderea numarului de rezultate fata de aceeasi cautare fara filtru.
...
**1. Gemini 3.6 Flash -> 3.7 Flash.** Nativ (id `gemini-3.7-flash`, confirmat in documentatia Google) si pe rutarea OpenRouter (`google/gemini-3.7-flash`). Cheie interna noua versionata `gemini-flash-3.7` (acelasi precedent ca la 3.6): cheia `gemini-flash-3.6` dispare din catalog, deci requesturile directe cu ea primesc 400 UNKNOWN_MODEL, iar selectiile din UI se re-mapeaza automat pe primul model disponibil. Fara migratie SQL — `CHECK`-ul din `ai_usage` e pe provider, nu pe modelId, iar selectia de model nu se persista. `AI_MAX_TOKENS` si allowlist-ul de effort raman neatinse: 3.7 are acelasi regim de reasoning ca 3.6 (mandatory, default enabled, effort medium). `callGoogle` se schimba, dar din alt motiv, independent de refresh — vezi punctul 3.

**2. Tariful lui 3.6 Flash raporta de doua ori peste real.** Tabelul de preturi avea 1.5/7.5 USD per 1M tokeni in/out — valorile care intra in vigoare abia la 1 ianuarie 2027. Tariful curent, promotional pana la 31 decembrie 2026, e 0.75/3.75 si e identic pentru 3.6 si 3.7. Corectat pe ambele tabele, cu comentariu explicit ca valorile trebuie bumpuite manual la 1 ianuarie 2027 (tabelul nu are dimensiune temporala). Fara backfill: costul se calculeaza si se stocheaza pe rand la insert, deci randurile deja scrise raman cu valoarea veche, iar un `UPDATE` pe istoricul de cost nu justifica riscul. Pe tabelul OpenRouter s-a pus tot pretul de LISTA, nu headline-ul de moment din catalog (3.7 apare azi la 0.375/1.875, dar acela vine de pe un endpoint cu `discount: 0.5`, iar catalogul listeaza sase endpointuri intre 0.1875 si 1.35 input) — tabelul acela e oricum doar fallback, fiindca ruta OpenRouter stocheaza costul real primit in `usage.cost`.

**3. Tokenii de thinking nu intrau in costul rutei native Google (descoperire colaterala).** `callGoogle` lua output-ul exclusiv din `candidatesTokenCount`, iar referinta API Google spune explicit ca `totalTokenCount = prompt + thoughts + candidates` — deci campul acela EXCLUDE tokenii de thinking, pe care Google ii factureaza la tariful de output. Consecinta: costul raportat iesea sub cel real pe toate modelele Gemini cu reasoning (3.5 Lite, 3.6 si 3.7 Flash, 3.1 Pro), nu doar pe cel nou, iar in cazul extrem — model care consuma bugetul pe thinking si nu mai emite text — un call platit se inregistra cu cost 0. `usageOutput` insumeaza acum `candidatesTokenCount + thoughtsTokenCount`, pastrand `undefined` cand lipsesc ambele campuri — distinctia conteaza, fiindca `0` afirma "nu a produs output" iar absenta spune "nu stim". Aceeasi corectie s-a aplicat si pe calea de eroare din `withAiLogging`, care extrage usage-ul best-effort din exceptie si citea si ea doar `candidatesTokenCount`: un call platit si esuat isi pierdea exact partea scumpa. Afectate erau raportarea de consum si pragul de avertizare pe buget; factura reala la Google si ruta OpenRouter (care primeste costul de la provider) au fost mereu corecte. Fara backfill, acelasi rationament ca la punctul 2.

**4. oauth2-proxy v7.7.1 -> v7.15.3 (securitate, doar deployuri de server).** Pinul din toate cele trei fisiere de compose ruleaza sub 7.15.2, unde se aduna sapte advisories publicate, trei critice. Trei dintre ele cad exact pe configuratia folosita aici, `skip_auth_routes` plus `reverse_proxy`: CVE-2026-40575 (bypass de autentificare cu `X-Forwarded-Uri` spoofat), CVE-2025-54576 si CVE-2026-41059 (bypass de `skip_auth_routes` prin query string, respectiv fragment). Impactul real e limitat — backend-ul isi cere singur autentificarea in web mode, deci cadea doar stratul exterior — dar operatorii dinainte de v2.45.0 trebuie sa redeployeze ca sa ia imaginea noua. Singurul breaking change intre 7.7 si 7.15 care atinge configuratia noastra e cel din v7.11.0 (`skip_auth_routes` se evalueaza doar pe path, fara query string), iar rutele livrate sunt ancorate pe path, deci comportamentul nu se schimba. Acelasi release a adus si `OAUTH2_PROXY_TRUSTED_PROXY_IPS`: nesetat, proxy-ul are incredere in `X-Forwarded-*` de la orice sursa si avertizeaza acum la pornire — comportamentul tacut al tuturor versiunilor anterioare, doar ca vizibil. Nu il putem seta noi (valoarea corecta e adresa proxy-ului fiecarui operator, iar una gresita blocheaza loginul), deci e documentat ca pas de hardening in `DEPLOY-NAS.md` si comentat in `deploy/.env.nas.example`. Conteaza mai ales pe NAS, unde portul 4180 e publicat pe LAN.

**5. Ruta OpenRouter livra ca analiza completa un raspuns oprit de filtru sau de eroare (bug preexistent, gasit la review).** v2.43.3 adusese detectia de trunchiere pe toate cele patru cai de provider, dar pe OpenRouter gate-ul ramasese doar pe `finish_reason === "length"`, in timp ce Anthropic, OpenAI si Google trateaza orice motiv de oprire diferit de `stop` ca taiere. Consecinta: un raspuns oprit de `content_filter` sau de o eroare de provider ajungea la utilizator ca analiza valida, fara niciun semn ca e incompleta. Adus la aceeasi regula ca celelalte trei rute. Absenta campului ramane deliberat netratata ca taiere — unii provideri din spatele OpenRouter il omit pe raspunsuri reusite, iar un gate pe absenta ar fi respins analize bune.

**6. `APP_VERSION` devine obligatoriu in cele trei compose-uri.** Default-ul tacut (`${APP_VERSION:-2.43.0}`, respectiv `:-2.43.3`) ramanea in urma la fiecare release si eticheta imaginea noua cu o versiune veche, deci tag-ul mintea la rollback si la audit — capcana s-a materializat deja o data pe deployul de pe NAS. Variabila e acum ceruta explicit (`${APP_VERSION:?...}`): fara ea, comanda de compose se opreste cu un mesaj clar in loc sa construiasca o imagine gresit etichetata. `deploy/.env.nas.example` a fost dus la 2.45.0.

**6. Documentatia de deploy aliniata la realitate.** Ruta PAT de pe NAS (`DEPLOY-NAS.md` §4b) e deja instalata si verificata din exterior, nu o procedura in asteptare — sectiunea o spune acum explicit si pastreaza pasii pentru un stack nou. Probele de acceptanta au fost extinse de la patru la sapte (exclusia bridge-ului de identitate, o ruta in afara capabilitatilor tokenului, o cautare RNPM reala) si insotite de tabelul de interpretare a codurilor de eroare. Doua capcane de diagnostic documentate: `/health` raspunde 404 pe hostname-ul de API prin design, iar matcher-ul compara `Bearer` case-sensitive, deci un client care trimite `bearer` cu litera mica primeste 404 fara niciun mesaj (verificat empiric). Corectat si un comentariu fals din `deploy/Caddyfile.pat`, care trimitea la o ruta `@pat` inexistenta in `deploy/Caddyfile`.

## v2.44.0 - 2026-08-02

Release de fiabilitate pe cautarea RNPM, pornit de la doua incidente reale pe deploy-ul web: eroarea intermitenta "Eroare server (502)" si, separat, avize afisate cu status "Necunoscut" pentru ca detaliile lor nu ajunsesera niciodata. Trei directii, fiecare cu test scris inainte de cod si demonstrat ca pica fara fix, plus review extern (fable-advisor, Codex, CodeRabbit) inainte si dupa implementare.

**1. Cautarea raspunde in flux, ca proxy-ul sa nu mai taie cererea.** Cauza celor 502: oauth2-proxy inchidea cererea daca headerele raspunsului nu soseau in 30 de secunde (`ResponseHeaderTimeout`), iar o cautare RNPM tine conexiunea deschisa pana termina captcha plus toate detaliile — frecvent peste un minut. Ruta `POST /api/rnpm/search` negociaza acum `Accept: text/event-stream` si trimite headerele imediat, apoi ping-uri la 10 secunde, iar rezultatul complet vine intr-un singur eveniment la final. Interfata nu se schimba: rezultatele apar tot deodata, nu picurate. Calea sincrona JSON ramane intacta pentru orice client care nu cere flux (test de caracterizare pe forma raspunsului). In paralel, `OAUTH2_PROXY_UPSTREAM_TIMEOUT` a fost urcat la 90s in toate cele trei fisiere de compose, ca plasa pentru clientii care nu folosesc fluxul.

**2. Detaliile care esueaza se reincearca automat, o data, la finalul cautarii.** Masurat in incident: 24 din 25 de avize pierdute integral intr-o cautare, iar un minut mai tarziu aceeasi cautare a adus 25 din 25 in 6,6 secunde — episoadele de lentoare la RNPM sunt tranzitorii. Pierderea era totala pentru ca detaliul unui aviz inseamna cinci cereri in `Promise.all`: cand istoricul cadea, lua cu el si partile 1-4, iar avizul nu se salva deloc si aparea in tabel ca "Necunoscut", nedistinctibil de un aviz al carui status chiar e necunoscut la sursa. Recuperarea ruleaza server-side, fara niciun buton si fara cost de captcha (cererile de detaliu merg pe uuid, nu pe gcode), cu plafon propriu de timp (`RNPM_RECOVERY_BUDGET_MS`, default 120s, `0` dezactiveaza). Ce ramane esuat si dupa recuperare e acum vizibil in interfata ("N avize fara detalii", cu identificatorii in tooltip) si raportat explicit in audit, inclusiv cat a sarit plafonul (`skipped`) — o taiere tacuta ar fi aratat identic cu "am incercat tot".

**3. Expirarile spun ce a cazut si nu mai ard bugetul degeaba.** Investigatia incidentului a durat o zi in mare parte pentru ca orice expirare producea acelasi text generic, fara sa spuna daca a cazut o parte sau istoricul; acum sunt etichetate (`part=3`, `istoric`). Al doilea defect din acelasi loc: cele doua incercari la istoric imparteau UN SINGUR buget, deci daca prima il consuma, a doua pornea cu semnalul deja expirat — fiecare incercare primeste acum buget propriu, iar o prima incercare agatata primeste a doua sansa (inainte se reincerca doar pe raspuns 400). Bugetul acopera cererea intreaga, deci si citirea corpului raspunsului: reincercarea si eticheta acopera acum acelasi interval, altfel un raspuns ale carui headere sosesc la timp dar al carui corp se scurge lent se pierdea la fel de complet. Distinctia expirare/abort e pastrata riguros — abortul clientului nu se reincearca niciodata, iar `readResponseTextWithCap` propaga motivul real al semnalului in loc sa sintetizeze `AbortError`, altfel o expirare in faza de corp se prezenta ca "clientul a plecat" si oprea toata cautarea.

Plus: abortul care ajunge in coada pe lock-ul de mentenanta nu mai lasa scrierea sa treaca (verificare de abort dupa achizitia lock-ului, nu doar inainte de coada).

## v2.43.4 - 2026-07-28

Primul release de dupa mutarea aplicatiei pe deploy web propriu (Synology, in spatele Cloudflare Tunnel + oauth2-proxy). Trei livrari, toate cu plan scris, review adversarial inainte si dupa implementare (fable-advisor si Codex), si verificare pe instanta reala.

**1. Buton de delogare in modul web (feature nou).** Interfata nu avea niciun control de delogare: backend-ul expunea `POST /api/v1/auth/logout` din v2.31.0, dar frontend-ul nu il apela niciodata. Pe desktop nu conta (un singur utilizator local); expusa public, aplicatia nu putea fi parasita de pe un calculator strain decat stergand manual cookie-urile. Butonul (doar in web, `sidebar-footer`) revoca JWT-ul pe server, apoi navigheaza la `/oauth2/sign_out?rd=%2Fdelogat`. Ruta noua `GET /delogat` (`routes/logoutPage.ts`, montata inaintea fallback-ului SPA, publica prin `SKIP_AUTH_ROUTES`) confirma delogarea, sterge defensiv cookie-ul de sesiune si avertizeaza vizibil ca sesiunea Google a ramas activa in browser. Doua capcane inchise pe baza review-urilor: `rd` absolut e respins de `OAUTH2_PROXY_WHITELIST_DOMAINS` (verificat live — devine `/`, de unde Google re-logheaza instant), iar `beginLogout()` asteapta un re-mint deja pornit, altfel acesta scria un JWT nou, cu jti nerevocat, DUPA stergerea cookie-ului. Raspunsurile non-2xx la logout sunt semnalate explicit (`fetch` nu arunca pe 403/429/500).

**2. Auditul inregistreaza IP-ul real, nu al proxy-ului.** In spatele lantului Cloudflare -> tunel -> oauth2-proxy, fiecare rand de audit primea adresa containerului vecin (`172.20.0.3`), iar intrarile de login nu primeau nimic — jurnalul era inutilizabil intr-o investigatie. Trei cauze distincte: `readClientIp` prefera acum `CF-Connecting-IP` (pus de Cloudflare la margine, nerescris de niciun hop, spre deosebire de `X-Forwarded-For`) cand peer-ul e deja de incredere; `readContext` din `auditRepository` folosea adresa socketului si ocolea toata logica de proxy; iar pe `/oauth2/sync` si `/logout` contextul nu poate fi pasat lui `recordAudit` (`getOwnerId` arunca pre-autentificare), deci IP-ul se trimite explicit. Verificat pe deploy: login-ul inregistreaza adresa reala a vizitatorului.

**3. Deploy pe NAS documentat.** `DEPLOY-NAS.md` + `deploy/docker-compose.nas.yml` + `deploy/.env.nas.example`: stack fara Caddy (TLS terminat de Cloudflare), oauth2-proxy publicat pe 4180, backend doar pe reteaua interna, SQLite pe bind mount. Doua bug-uri gasite la primul deploy real, valabile si pentru varianta VPS: `scripts/seed-admin.mjs` lipsea din imagine (comanda de provizionare a adminului din `DEPLOY-SERVER.md` esua cu "Cannot find module"), iar digest-ul pinuit pentru `oauth2-proxy:v7.7.1-alpine` era gresit (quay serveste `1a2009d3...`, nu `36b46309...`), deci pull-ul esua pe orice masina fara imaginea deja in cache — corectat si in `docker-compose.prod.yml` si in `infra/docker-digests.md`.

## v2.43.3 - 2026-07-26

Release de securitate + calibrare AI, pe acelasi branch `feat/v2.43.0-rnpm-split`. Patru directii, fiecare cu plan scris, review adversarial (Codex si/sau fable-advisor) si implementare TDD; doua runde suplimentare de review adversarial pe intreaga livrare. Canary live pe OpenRouter executat si reconciliat la cent cu dashboard-ul provider-ului.

**1. Securitate — cele 3 findings care blocau web deploy (Faza 12).** F12-F8: PAT-urile (`/api/v1/tokens*`) se emit si se administreaza DOAR de admin — gate-ul era doar in UI, orice user autentificat putea emite tokenuri. F12-F5: cheia captcha a tenantului nu mai ajunge in raspunsuri sau loguri — redactare la constructia erorii (forma bruta + percent-encodata), corpul 500 devine generic pe toate cele sase sinkuri; intarita post-release-review si pe `cause` (eroarea SDK bruta se reconstruieste cu name + message/stack redactate, proprietatile extra si cause-ul imbricat se arunca — protejeaza si logging-ul object-aware viitor). F12-F3: limita de stocare RNPM nu mai poate fi ocolita cu `gcode` din body — masuratoarea se face inainte de admitere pe toate rutele de cautare. Plus CodeRabbit 1.2: gardul de restore ruleaza inainte de consumul captchei (dupa rezolvarea configului, ca 501-ul canonic din web mode sa se pastreze), replicat pe `/search`, `/bulk` si `/search-split` — un 409 nu mai costa o captcha.

**2. Calibrare AI pentru Claude 5 (effort + plafoane + cost real).** Pe generatia Claude 5 thinking-ul e implicit si se factureaza ca output: plafonul de tokeni e acum SPLIT — `AI_MAX_TOKENS` ramane 8000 pentru modelele fara effort (GPT, Gemini, Haiku), `AI_MAX_TOKENS_EFFORT` = 16000 doar pentru Claude 5, care primeste si effort redus prin allowlist exact (`claude-sonnet-5`/`claude-opus-5`, nativ prin `output_config.effort`, OpenRouter prin `reasoning.effort` pe slugul REZOLVAT dupa override-uri): `low` pe analisti si pe analiza single, `medium` pe judecator. Fix bug preexistent: `callOpenRouter` trimitea `extra_body` (idiom din SDK-ul Python, ignorat de OpenRouter) — `usage: { include: true }` si `reasoning` sunt acum top-level, deci raportarea de cost real de la provider functioneaza pentru prima data (validata live: `costUsdMilli` din stdout reconciliat exact cu dashboard-ul OpenRouter pe toate cele 3 apeluri de canary). Observabilitate in acelasi release: `stop_reason`/`finish_reason` logat pe toate rutele, detectia de trunchiere extinsa pe TOATE cele patru cai de provider (GPT si Gemini livrau analize taiate ca rezultate complete; opririle Gemini SPII/BLOCKLIST/OTHER returnau text partial in tacere) cu degradare uniforma, eroare tipizata pe raspuns gol pe fluxul single, analizele deja platite se pastreaza cand judecatorul cade, usage-ul partial se recupereaza la erori de stream. Kill switch operational `AI_EFFORT_DISABLED=1`. `TRUNCATE_ANALYSIS` 50k → 120k caractere (analiza de 16k tokeni nu mai e taiata inainte de judge).

**3. Restul findings-urilor CodeRabbit (review pe 54 + review pe 31).** Rollback-ul migratiei 0041 nu mai dubleaza grantul AI; pre-migrarea bazelor RNPM se face la boot, nu pe prima cerere (fara latenta pe primul request); garda de dublu-click pe Backups pe ref sincron; paginare + filtru server-side pe `/admin/rnpm/usage`; kill switch-ul CSRF logheaza la boot; autocompactarea sarita isi logheaza motivul si cand pica masuratoarea; RUNBOOK-ul de restore spune explicit sa opresti aplicatia; lot cosmetic verificat individual. Remedieri din reviewurile adversariale pe implementare: deadlock de maintenance lock introdus in `/usage` (RWLock nereentrant) eliminat cu varianta unlocked; masuratoarea peste limita nu mai lasa handle-uri deschise; delete-all curata si jail-ul de backup; fisier de test cu byte NUL care nu aparea in diff-uri reparat.

**4. Stergere individuala de backup (feature nou, admin).** `DELETE /api/v1/admin/backups/:name` + buton "Sterge" per rand in Setari > Backup, cu confirmare destructiva: refoloseste jail-ul de nume al restore-ului (validare inainte de lock) si write lock-ul de mentenanta al delete-all, audit `backup.delete` cu targetId si outcome, sidecars best-effort. Plan reviewuit adversarial (fable-advisor + Codex, perimetru inchis) inainte de implementare, review final pe implementare, smoke live din UI in web mode.
...
Acelasi refresh si pe slotul "Rapid" Google (adaugat 2026-07-22, in acelasi release): Gemini 3.1 Flash Lite -> 3.5 Flash Lite, nativ (id `gemini-3.5-flash-lite`) si pe OpenRouter (`google/gemini-3.5-flash-lite`). Cheie interna noua versionata `gemini-flash-lite-3.5`, cheia veche `gemini-flash-lite-3` retrasa din catalog cu aceeasi re-mapare automata a selectiilor salvate. Pricing 0.3/2.5 USD per 1M tokeni in/out (verificat in catalogul live OpenRouter 2026-07-22; 3.1 Lite era 0.1/0.4) — ramane cel mai ieftin model din catalog, iar intrarea 3.1 ramane in tabelul de pricing pentru retry-urile in zbor. Aceleasi actualizari de manual, DOCUMENTATIE.md si teste de regresie ca la refresh-ul Flash.

Refresh si pe slotul "Premium" Anthropic (adaugat 2026-07-25, in acelasi release): Claude Opus 4.8 -> Claude Opus 5, nativ (id `claude-opus-5`) si pe OpenRouter (`anthropic/claude-opus-5`). Spre deosebire de refresh-urile Gemini, cheia interna ramane `claude-opus` (precedent Sonnet 4.6 -> 5): nicio cheie nu se retrage din catalog, deci requesturile care o trimit raman valide (fara 400 UNKNOWN_MODEL) si nu e nevoie de re-mapare de selectii, `JUDGE_MODELS` si default-ul judecatorului din multi-agent nu se ating, iar utilizatorul nu trebuie sa reconfigureze nimic. Pricing 5/25 USD per 1M tokeni in/out, identic cu 4.8 (verificat 2026-07-25), deci cotele per utilizator nu se recalibreaza; intrarea 4.8 ramane in tabelul de pricing pentru retry-urile in zbor si pentru un eventual rollback prin `OPENROUTER_MODEL_OVERRIDES`. Istoricul `ai_usage` se imparte natural pe id-uri de model, nu pe cheia interna: `claude-opus-4-8` / `claude-opus-5` pe ruta nativa, respectiv `anthropic/claude-opus-4.8` / `anthropic/claude-opus-5` pe OpenRouter. Corectat in acelasi timp un mesaj de eroare stale: modelele judecator permise se enumera acum ca "Claude Opus 5, GPT-5.6 Sol si Gemini 3.1 Pro" (spunea inca "GPT-5.4", desi catalogul trecuse pe 5.6 Sol in v2.42.x). Actualizate manualul in-app, exportul PDF al manualului si tabelul de modele din DOCUMENTATIE.md; teste de regresie pe slug-ul OpenRouter si pe etichetele din UI.
...
### Fixuri post-review adversarial (pre-merge)

Inainte de merge, delta a trecut prin doua review-uri adversariale independente (Codex GPT-5.6 Sol + panel multi-model), iar findings-urile confirmate au fost inchise pe TDD in 3 commit-uri consolidate:

**Restore atomic prin staging.** Restore-ul (monolit + RNPM) materializeaza backup-ul intai intr-un director de staging, unde e verificat (integrity check) si checkpoint-uit intr-un singur fisier self-contained; fisierul viu e atins abia la publicare, printr-UN SINGUR rename atomic cu retry pe erori tranzitorii Windows. Esec inainte de publicare = fisier live byte-identic; esec la proba post-publicare = auto-revert reordonat fail-safe (sidecar-urile straine dispar inaintea revert-ului). Staging-ul orfan de la un crash anterior e curatat. Restore-ul RNPM devine fail-closed pe lipsa `_schema_versions` (jail-urile exista doar din v2.43.0); monolitul primeste aceeasi validare de versiune, dar accepta backup-urile legacy fara tabela.

**Serializare, retentie si cooldown.** Stergerea tuturor backup-urilor (monolit + RNPM) intra sub maintenance lock (nu mai poate rula peste un restore in zbor); prune-ul de retentie ruleaza si la restore (snapshot-urile pre-restore raman plafonate la 5 sub un loop de restore-uri); backup-urile pre-split primesc pool DEDICAT de retentie (3, cu excludere regex din pool-ul pre-migration — nu mai erau evacuate primele) si prune sincron la boot (un crash-loop la split nu mai umple discul); cooldown-ul backup-ului manual ramane setat la start (anti-double-submit) dar se refundeaza la esec; `ownerId` invalid de la admin pe rutele de backup raspunde 400, nu 500.

**Marker de split fail-closed + manifest.** Marker-ul `.split-done.json` e validat runtime strict (status necunoscut, owners invalizi sau manifest malformat = boot abortat cu instructiuni RUNBOOK — inainte, un status necunoscut cadea pe fluxul de split normal si putea suprascrie fisiere per-user mai noi). Split-ul scrie in marker-ul `wiping` un manifest cu count-urile verificate per owner/tabela; reluarea golirii re-verifica fisierele per-user (existenta + integritate + count-uri identice) inainte sa goleasca monolitul.

**Ordinea de boot si shutdown.** In web mode, gate-ul de master key (`TENANT_KEY_ENCRYPTION_SECRET` + proba round-trip) ruleaza INAINTE de split (un secret lipsa nu mai lasa monolitul golit la abort); schema init are atributie proprie de eroare. La shutdown, scrierile de mentenanta noi sunt refuzate tipat (503 + `Retry-After` prin handlerul central), iar cele in zbor — inclusiv cele care asteapta pe lock — sunt asteptate pana la 30s printr-un settle-set.

**Paritate monolit la restore + 409 corect.** Restore-ul monolitului are latch global pe `getDb()` (toate rutele raspund 409 `RESTORE_IN_PROGRESS` pe durata swap-ului — indisponibilitate globala scurta, admin-triggered, documentata in RUNBOOK), cu snapshot pre-restore anti-self-block. Erorile tipate de concurenta nu mai sunt inghitite de catch-urile generice ale rutelor (`POST /compact` sub restore = 409, nu 500).

**Igiena web-mode.** Prewarm-ul RNPM de la boot ruleaza doar pe desktop (fara fisier `rnpm/local-*.db` orfan in web mode); `GET /stats` si `POST /compact` nu mai provisioneaza fisierul unui user care nu a folosit RNPM (zerouri, respectiv 404); warn structurat la boot cand `LEGAL_DASHBOARD_TRUSTED_PROXY_CIDR` lipseste in web mode (+ documentatie CSRF/rate-limit in DEPLOY-SERVER si RUNBOOK).

**VACUUM in worker thread.** Snapshot-urile (daily, manual, pre-restore) si compactarea ruleaza prin `VACUUM INTO` intr-un worker thread (`snapshot-worker.cjs`, cu timeout hard, terminate pe toate caile si fallback sincron daca worker-ul nu porneste) — event loop-ul nu mai e blocabil de operatii pe baze mari. Compactarea per user devine swap atomic sub maintenance lock (fisierul e inchis, worker-ul scrie copia compactata, rename + redeschidere lazy), sub acelasi latch per owner ca restore-ul — validat cu advisorul Codex: inchide complet fereastra de pierdere de scrieri concurente. Testul anti-drift compara acum si definitiile indexurilor/triggerelor (SQL normalizat + `index_xinfo`, inclusiv autoindexurile UNIQUE).

**Rev. 5 (focus deploy web).** Cele doua HIGH ramase din review-ul de inchidere: la retentie, un backup al carui fisier principal nu poate fi sters isi pastreaza acum si sidecars-ul WAL/SHM intact (bundle-urile legacy au date comise doar in WAL — inainte, "recovery point-ul pastrat" putea ramane corupt silentios), iar recuperarea unui lock de instanta mort trece printr-un gate creat atomic (`O_EXCL`) cu re-evaluare completa sub gate si self-heal pe gate orfan — doua porniri concurente pe acelasi volum (docker restart, redeploy) nu mai pot prelua amandoua lock-ul; pierzatorul refuza fail-closed cu mesaj de reincercare.

**Runda de inchidere (Rev. 4, review Codex pe intreg branch-ul).** Securitatea si arhitectura au iesit curate la review-ul adversarial pe tot branch-ul; s-au inchis ultimele findings de rezilienta si cod: lock-ul de instanta nu mai recupereaza automat un PID viu de pe acelasi host (operatiile sincrone de boot — split, migratii — pot bloca legitim heartbeat-ul peste prag; proba de proces devine fail-closed, doar ESRCH inseamna "mort", iar mesajul de refuz explica situatia si indica break-glass-ul `LEGAL_DASHBOARD_FORCE_BOOT=1`), auditul de dupa un restore reusit nu mai poate rasturna rezultatul in 409/500 (recordAuditSafe pe caile de succes, rnpm + admin), backup-urile manuale primesc nume unic cu milisecunde si sufix incremental plafonat (doua create-uri in aceeasi secunda nu se mai suprascriu silentios), iar retentia numara doar stergerile reale si semnaleaza structurat unlink-urile refuzate de AV/ACL (`backup_prune_failed`).
...
### Fixuri si admin post-split (2026-07-12, validate in human testing)

**Captcha fallback in web mode.** `withRnpmCaptchaGuards` propaga a doua cheie tenant ca `fallback2CaptchaKey` — race-ul si fallback-ul secvential intre 2Captcha si CapSolver functioneaza si pe web, nu doar pe desktop (gap observat live la un ban de proxy CapSolver: `ERROR_PROXY_BANNED` absorbit de 2Captcha fara 500). Rutele folosesc exclusiv fallback-ul din guard (`body.fallback2CaptchaKey` nu mai face BYOK din browser in web mode), iar abort-ul clientului in timpul race-ului se propaga ca 499, nu 500. Risc acceptat documentat: in race, ambele portofele consuma cand ambii provideri termina.

**Buton "Sterge backup-urile" per user in cardul admin Stocare RNPM.** Endpoint-ul cross-owner existent (audit `backup.rnpm.delete_all`), confirmare destructiva cu count + dimensiune, guard sincron anti double-click partajat cu Compacteaza; functioneaza si pentru userii stersi care mai au backup-uri pe disc (protejat de regresie cu test dedicat).

**Batch 2 CodeRabbit (confirmate cu Codex).** Politica de cache pe path-ul normalizat (un traversal encodat sub `/assets/` nu mai primeste `immutable` pe `index.html`), `USAGE_OVERVIEW_CAP` aplicat si pe boundary, fallback `quotaPeriodLabels` pe conventia `Necunoscut (token)`, test full-flow cu rand post-backup real + assert uuid.

### Autocompact + limite de stocare RNPM (2026-07-12/13, implementare Codex GPT-5.6 + audit 4 agenti)

**Autocompact dupa stergeri.** Stergerea de avize (individuala, batch sau totala) masoara freelist-ul fisierului RNPM al ownerului si, la prag (>=10 MB liber SI >=20% din fisier), compacteaza sincron prin VACUUM INTO + swap atomic, sub maintenance write lock + latch-ul de restore, cu recheck sub lock (cereri concurente coalesced, nu dublu-VACUUM). Esecul compactarii nu atinge stergerea deja comisa (`compacted:false` + reason in audit `rnpm.autocompact`). Validat live: delete-batch de 80 avize a eliberat 103.5 MB in 118ms. Kill switch `LEGAL_DASHBOARD_RNPM_AUTOCOMPACT_DISABLED=1`, prag ajustabil `LEGAL_DASHBOARD_RNPM_AUTOCOMPACT_MIN_FREE_MB`.

**Limita de stocare per user (default 750 MB).** Admission control pe `POST /rnpm/search`/`/bulk`/`/search-split`, plasat INAINTE de guard-ul captcha (refuzul nu consuma captcha): la `used >= limit`, 429 `QUOTA_EXCEEDED` cu `{feature, usedBytes, limitBytes}` si mesaj cu cifre + indicatia de stergere/compactare. Masurarea (db + wal + shm, sub maintenance read, cu second-chance checkpoint PASSIVE) e UNICA — acelasi numar in guard, `GET /stats` si cardul admin. Recheck in timpul cautarii: intre paginile interne, intre itemele bulk si intre sub-cautarile split (inclusiv nested tier-2); continuarile cu `gcode` existent sunt exceptate (sesiune platita, overshoot plafonat). Override per user din Setari > Cote: feature nou `rnpm.storage` in MB (`user_quota_overrides`, precedentul captcha.rnpm=count), override 0 = blocheaza, override sters = revine la default; default env `LEGAL_DASHBOARD_DEFAULT_RNPM_STORAGE_MB` (0/negativ = nelimitat). Restore-ul peste limita ramane permis (decizie: recuperarea primeaza).

**Plafon pe jail-ul de backup RNPM (500 MB, best-effort).** `pruneOld` capata faza de byte-cap DOAR pe jail-urile RNPM: evacuare oldest-first cross-pool cu PODEA — cea mai recenta copie din fiecare pool (dated/manual/pre-restore/pre-migration) nu se sterge niciodata; `capSatisfied` raportat onest din re-stat real (unlink partial pe sidecars = false + log structurat `backup_prune_failed`). Pool-urile RNPM reduse la 3/2/2/2. Pre-migration backup-ul per fisier RNPM intra in accounting si se auto-protejeaza. Env `LEGAL_DASHBOARD_RNPM_BACKUP_CAP_MB`.

**Fixuri din auditul post-implementare (4 agenti paraleli + gate independent).** Recheck-ul de limita din TIMPUL unei cautari simple iesea 500 generic in loc de 429 cu cifre (catch-ul local din `/search` nu rethrow-uia eroarea tipata spre handlerul central); bulk/split/nested nu se opreau la primul refuz de limita — pana la 199 de recheck-uri inutile per request, fiecare cu lock global + stat + checkpoint (acum fail-fast: itemele ramase sunt marcate 'Oprit: limita de stocare RNPM atinsa', fara increment fals de `captchasUsed` pe refuz pre-captcha). TDD strict: 4 teste RED inainte de fix.

### Human testing multi-user (2026-07-13, sesiune live cu 4 useri paraleli)

Test live cu 4 identitati simultane (admin + 3 useri, proxy-uri dev separate): 4 cautari RNPM in paralel, zero erori interne, zero contention vizibil pe lock-ul de mentenanta (search 0.7-2.6s constant), provisioning lazy per user corect (fisierele `rnpm/<stem>.db` create la prima cautare). Constatari externe documentate: RNPM throttling agresiv la volum paralel de pe acelasi IP (detalii 11s -> 90s per batch, apoi refuz `410 Gone` pe cautari noi, auto-ridicat dupa pauza) si paginare instabila sub sarcina (repeta randuri intre pagini: 150-175 livrate la un total real de 144).

**Fixuri directe din human testing:** lista de rezultate deduplica dupa identificator la incarcarea paginilor — contorul nu mai poate depasi totalul, randurile duplicate dispar (datele salvate au fost mereu curate: dedup by identificator la persistare, verificat 144/144 pe toti userii); cardul admin Stocare RNPM ascunde implicit userii stersi/suspendati fara date (checkbox 'Arata si userii stersi sau suspendati fara date (N)'; cei care inca ocupa spatiu raman mereu vizibili); limita implicita ridicata 500 -> 750 MB (decizie user).

**Backlog notat din testing (pre-lansare web):** shard pe owner pentru maintenance lock (azi global — compactarea unui user tine in asteptare guard-urile altora, cateva secunde, rar), ritm global politicos catre RNPM (4+ useri paraleli de pe un IP declanseaza apararea lor), mesaj prietenos pe refuzurile upstream (azi `Eroare RNPM search (410)` brut), pas obligatoriu de deploy: audit useri peste limita via `GET /api/v1/admin/rnpm/usage` + override-uri preventive inainte de cutover.
