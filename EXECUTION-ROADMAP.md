...
- **v1.18** (2026-05-03): patch v2.10.7 inregistrat - `Monitorizare` afiseaza `Joburi active (${total})` pe totalul real al listarii paginated, nu pe `jobs.length` din pagina curenta; tooltip-urile Excel/PDF clarifica exportul pe joburile vizibile; `CODEX-BACKLOG.md` inchis ca document istoric, cu Task B/C livrate si Task A eliminat.
- **v1.19** (2026-05-04): patch v2.10.8 inregistrat - CI hardening pe `.github/workflows/build-windows.yml` si `.github/workflows/build-mac.yml`: 4 pasi noi (`tsc --noEmit` backend, `vitest run` backend, `tsc --noEmit` frontend, `vitest run` frontend) inseratiti intre `npm ci` si `rebuild:electron` (Windows) / `npm run build` (Mac). Ordinea conteaza pentru ABI `better-sqlite3`: testele ruleaza cu ABI Node lasat de `npm ci`, inainte de flips la ABI Electron. Artifact naming aliniat cu pattern-ul `legal-dashboard-{platform}-${{ github.ref_name }}-run${{ github.run_id }}` ca rerun-urile pe acelasi tag sa nu mai cauzeze overwrite. Backlog-ul "GitHub Actions packaging" scos din docs.
- **v1.20** (2026-05-04): patch v2.11.0 inregistrat - deep-review remediation peste v2.10.8 (PR A operational + PR Web-Readiness Closure din `DEEP-REVIEW-LEGAL-DASHBOARD-2026-05-04.md`, exceptie: trecerea frontend `xlsx` → `exceljs` deferata). Securitate: `backend/rnpm-dumps/` adaugat in `.gitignore` (PII real RNPM); `nodemailer` `^6.9.13` → `^7.0.13` (HIGH DoS GHSA-rcmh-qjqh-p98v patched 7.0.11+); `@anthropic-ai/sdk` `^0.90.0` → `^0.92.0` (moderate file-perms GHSA-p7fg-763f-g4gf). Backend: `routes/rnpm.ts` propaga `ownerId = getOwnerId(c)` end-to-end pe `executeSearch`/`executeBulkSearch` + `inflightKey(ownerId, ...)`; `requireRole("admin")` pe rutele globale (`DELETE /saved/all`, `POST /compact`, `DELETE/GET/POST /backups{,/restore}`, `POST /open-{db,backups}-folder`); helper `rejectCaptchaKeyInWebMode()` returneaza 501 in `AUTH_MODE=web` pe `POST /search`/`/bulk`/`/captcha/balance`. Build: `scripts/build-server.js` rebrand "portaljust" → "legal-dashboard". Tests: 728/728 backend (+7 contract noi pe admin guard + web-mode 501 gate cu `updateUserRole("local","user")` in `beforeEach`). 73/73 frontend.
- **v1.21** (2026-05-04): patch v2.12.0 inregistrat - MIN-VIABLE seam refactors peste v2.11.0 (4 boundary-uri decuplate fara outbox tables / DI containers, plus fix paginare timeline Dashboard). AlertEventService: monitoring alerts merg prin `services/alerts/alertEventService.ts`, fanout email + SSE deferat prin `queueMicrotask` ca SQLite write lock-ul sa nu se mai tina peste IO extern; `drainEmailDispatches(2_000)` la shutdown. Command service: `POST /api/v1/monitoring/jobs` deleaga la `services/monitoring/createMonitoringJobService.ts` (input parsat de Zod la boundary HTTP + callback `writeAudit`, return union `created/duplicate/invalid`). Hook: `hooks/useMonitoringJobs.ts` cu AbortController pe unmount + flush sincron pe debounce. Notifications module: SSE subscriber set + native Electron toast extrase intr-un port simplu (swap server-side push compatibil). Bug fix: timeline `GET /api/v1/dashboard/activity/timeline` pierdea un eveniment per pagina cand cursorul cadea exact pe ts-ul partajat intre surse; fix `fetchLimit = inclusive ? limit + 1 : limit` (composite-ID-uri unice → +1 e suficient). Tests: 744/744 backend (+16 noi: +3 in `services/alerts/alertEventService.test.ts` cu vi.mock pe SMTP boundary + drain in afterEach; +13 distribuite intre `routes/rnpm.owner-isolation.test.ts` (11 owner-isolation pe DB partajata) si `routes/dashboard.test.ts` (compound cursor disambiguation)). 73/73 frontend.
- **v1.22** (2026-05-04): patch v2.12.1 inregistrat - UX bulk import + nume lungi PortalJust peste v2.12.0. Frontend: `MonitoringBulkImportCard.tsx` inlocuieste limita statica de 300 randuri vizibile cu paginare server-style identica cu pagina principala (default 100/pagina, `pageSizes=[25, 50, 100, 250]`); coloana noua "Actiune" cu Exclude/Include per rand; checkbox "Exclude warn-urile automat" pentru bulk; legenda colapsabila statusuri ok/warn/respins + nota dedup automat (constraint UNIQUE owner_id + target_hash + kind). Backend: `services/nameListParser.ts` `classifyRawName` rescris cu mesaje romanesti complete + regula noua `nume_lung` (warn) la `>100` chars sau `>12` cuvinte (constante `PORTALJUST_WARN_*` exportate, helper `isLikelyTooLongForPortalJust`); `services/monitoring/scheduler.ts` adauga helper `computeProbableCause(job, outcome)` care, pentru `name_soap` cu `errorCode === "SOAP_FAIL"` si nume peste limite, marcheaza alerta `source_error` cu `probable_cause: nume_prea_lung_pentru_portaljust` + titlu specific + detail JSON `{nameNormalized, length, wordCount}`. Tests: 751/751 backend (+4 in `nameListParser.test.ts` warn nume lung cu exemplul real GLOBALSAT din raportul utilizatorului; +3 in `scheduler.test.ts` probable_cause enrichment). 73/73 frontend.
- **v1.23** (2026-05-05): release v2.13.0 inregistrat - Export alerte (Excel/PDF) + raport zilnic email peste v2.12.1. Backend: nou `routes/alerts.ts` `POST /api/v1/alerts/export` cu Zod `discriminatedUnion("mode", [ids|filters|range])`, cap 10k randuri (413 + total in details), reuseaza `deriveAlertDigestRow` pentru decorare cu `numarDosar/dosarLink/kindLabel/severityLabel/nameMonitored`; audit `alerts.export` cu `mode + count`. Migration `0015_daily_report_settings.up.sql` adauga in `owner_email_settings`: `daily_report_enabled INTEGER NOT NULL DEFAULT 0` + `last_daily_report_sent_for TEXT NULL`. Nou `services/email/dailyReportTemplate.ts` cu `renderDailyReport({reportDateLocal, alerts})` + `getPortalJustUrl` + `deriveAlertDigestRow`; `services/email/dailyReportScheduler.ts` cu `runDailyReportTick(deps?)` care fires doar la ora locala configurabila (`DAILY_REPORT_HOUR=9` default), itereaza prin owneri cu flag activ + `last_daily_report_sent_for != today`, fereastra `[yesterday 00:00, today 00:00 local)`, audit `email.daily_report.sent`/`.failed` + marcare zi doar pe success. `mailer.sendComposedEmail` reutilizat; `index.ts` porneste `setInterval` 5 min. Frontend: `lib/alertsApi.exportAlerts(payload)`, `lib/export-alerts.ts` (`buildAlertsXlsx` cu hyperlink portal.just.ro live + `buildAlertsPdf` cu pdfmake link), `pages/Alerts.tsx` checkbox per rand + buton "Exporta" + modal radio Excel/PDF, `components/EmailSettingsPanel.tsx` checkbox "Trimite raport zilnic la 09:00" controlat de `dailyReportEnabled`. Tests: 789/789 backend (+38: 17 template + 12 scheduler + 7 export route + 2 me daily flag); 81/81 frontend (+8: 3 alertsApi exportAlerts + 3 export-alerts + 2 EmailSettingsPanel daily flag).
