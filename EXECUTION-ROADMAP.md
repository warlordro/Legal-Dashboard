...
- **v1.18** (2026-05-03): patch v2.10.7 inregistrat - `Monitorizare` afiseaza `Joburi active (${total})` pe totalul real al listarii paginated, nu pe `jobs.length` din pagina curenta; tooltip-urile Excel/PDF clarifica exportul pe joburile vizibile; `CODEX-BACKLOG.md` inchis ca document istoric, cu Task B/C livrate si Task A eliminat. Findings-urile de workflow metadata / release artifact naming raman deferate pentru o sesiune separata.
- **Backlog tehnic viitor - GitHub Actions packaging**: in `.github/workflows/build-windows.yml` si `.github/workflows/build-mac.yml`, adauga type-check + teste inainte de packaging. Aliniaza artifact naming intre Windows/macOS si evita numele fixe (`legal-dashboard-windows`, `legal-dashboard-mac`) prin includerea versiunii/ref-ului si a `github.run_id`, ca run-urile tag/manuale sa fie usor de distins si sa nu apara overwrite-uri.
