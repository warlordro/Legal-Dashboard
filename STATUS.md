...
**Data curenta:** 2026-05-13 (v2.22.0: Batch 7 + Batch 8 din FIXES-TODO.md - supply chain hardening (GitHub Actions SHA pinning, Dockerfile digest pinning, xlsx -> xlsx-js-style pe path-ul de upload user, hono 4.12.18 cu 3 CVE inchise) + polish (`synchronous = NORMAL` pe SQLite, `RNPM_SITEKEY`/`RNPM_PAGEURL`/`RNPM_USER_AGENT` extragate in env via lazy getters) + Faza 1-3b migrare exporturi mari la `exceljs.WorkbookWriter` + `pdfkit` streaming server-side pentru RNPM, PortalJust dosare/termene si Alerte (commits 3b69e4c, f9d11ec, d600959, 9ece8ca) + hardening export streaming (body cap 25MB, type-guard payload, write-stream error race, helper TZ-safe `formatRoDate`/`formatRoDateTime`); 900 backend pass + 92 frontend pass)
**Versiune curenta reala:** v2.22.0
