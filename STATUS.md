...
**Data curenta:** 2026-05-17 (v2.28.2: Bugfixes + UX tuning + ops watchdog peste auditul de refactor Tier 1+2 (13 commits livrate ca grup) — fix `[object Object]` la 400/SSE error in cautari RNPM (re-activeaza Split flow rupt din v2.14.0), fix native AI mode care ramanea ancorat pe OpenRouter dupa toggle (regresie v2.28.0), 3-state ownership pe searchRepository (`owned/foreign/missing`) ca sa traduca graceful searchId-ul cache-uit dupa "Sterge baza", event-loop watchdog Electron pe `node:perf_hooks.monitorEventLoopDelay` cu raporte JSON in `diagnostic/reports/`, plus headere AI strangite cu -33% inaltime + vertical-align pe rows Monitorizare + notite inline constrante la 12rem.)
**Versiune curenta reala:** v2.28.2
