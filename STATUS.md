...
**Data curenta:** 2026-05-10 (v2.20.6: hygiene release — `.env.example` reconstruit (~25 env vars in 7 sectiuni, inchide CP-2 din root CLAUDE.md) + `requireRole` envelope migration aditiva (`fail()` cu `data: null` + `requestId` pe 401/403); restul rutelor legacy raman amanate pentru PR-6 per policy `util/envelope.ts` + guard `rnpm.contract.test.ts`)
**Versiune curenta reala:** v2.20.6
