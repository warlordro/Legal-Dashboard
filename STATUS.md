...
**Data curenta:** 2026-05-16 (v2.27.5: fix performanta filtrare RNPM. Materializare cu migration 0022 — 24 coloane `*_norm` pe rnpm_avize/creditori/debitori/bunuri/bunuri_descrieri, populate de 10 triggere `AFTER INSERT/UPDATE OF`. Backfill idempotent post-migration in `schema.ts`. Read-path (`avizRepository.ts`) citeste direct `col_norm` in loc sa apeleze UDF-ul `rnpm_norm()` per rand x 24 coloane. Elimina freeze-ul de ~8s pe filtrare cu 148 rezultate raportat in productie. Zero regresie functionala — aceleasi 24 coloane match, acelasi pattern LIKE, acelasi highlightTokens UI. 7 teste regresie noi acopera populare triggere + scenarii diacritice/JSON/4-char prefix. Hard constraints respectate: SQL raw doar in backend/src/db/**, owner_id pe toate tabelele, DOMPurify activ, whitelist URL neatinsa.)
**Versiune curenta reala:** v2.27.5
