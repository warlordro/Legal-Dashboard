...
**Data curenta:** 2026-07-28 (v2.43.4: delogare in modul web (buton in sidebar, pagina de confirmare `/delogat`, cu avertisment ca sesiunea Google ramane activa) si auditul care inregistreaza adresa reala a vizitatorului in loc de adresa proxy-ului, plus deploy pe NAS documentat (`DEPLOY-NAS.md`) cu doua bug-uri de imagine reparate. Predecesor v2.43.3: securitate + calibrare AI — cele 3 findings F12 care blocau web deploy livrate (PAT admin-only, cheia captcha redactata inclusiv pe `cause`, limita de stocare RNPM neocolibila prin `gcode`), effort si plafoane split pentru Claude 5 cu raportarea costului real OpenRouter reparata si validata live, detectia trunchierii pe toate cele 4 rute de provider, fixurile CodeRabbit + doua runde de review adversarial, stergere individuala de backup admin. Predecesor v2.43.2: refresh modele AI, nativ + OpenRouter — Gemini 3.5 Flash -> 3.6 Flash pe slotul "Echilibrat" (cheie interna noua `gemini-flash-3.6`), Gemini 3.1 Flash Lite -> 3.5 Flash Lite pe slotul "Rapid" (cheie interna noua `gemini-flash-lite-3.5`) si Claude Opus 4.8 -> Opus 5 pe slotul "Premium" (cheia interna `claude-opus` ramane, acelasi pricing 5/25). Predecesor v2.43.1: remedierea auditului de securitate/corectitudine si rezultatele partiale la cautarea PortalJust, dublu-review-uite — vezi CHANGELOG v2.43.1. Predecesor v2.43.0: split RNPM per utilizator + autocompact dupa stergeri si limite de stocare per user (750 MB default, override rnpm.storage din Cote, plafon 500 MB pe jail-ul de backup) — fisier SQLite propriu `rnpm/<stem>.db` per cont, splitter one-time crash-safe cu marker durabil si backup pre-split verificat, backup/restore self-service pe jail-ul propriu cu garduri de concurenta, backup multi-target cu VACUUM INTO si retentie pe pool-uri, tab Setari > Backup admin-only pentru monolit. Predecesor v2.42.0: administrare utilizatori completa — creare + import Excel + reactivare conturi sterse, pagina /setari pe taburi, pool AI unic cu cote si granturi, tab Consum per utilizator, audit cu email + export XLSX, Sonnet 5 + prompturi system/user, finisaj UX Nivel 1+2 (toast-uri, confirmari, sortare, dark mode) si aliniere design la repo-ul de referinta. Predecesor v2.40.0: API programatic doar-citire prin Personal Access Tokens — Piesa A din planul API + MCP, doar web mode. Dosare + termene (PortalJust), ICCJ si RNPM accesibile extern cu tokenuri `ld_pat_...` (hash SHA-256 in DB, migratia 0039); gate default-deny pe tuple (metoda, path, scope), management session-only, HTTPS-only + no-store, rate-limit + plafon captcha per-token (rezervare atomica fail-closed), audit + alerta email la IP nou, circuit breaker ICCJ ponderat pe caller class, OpenAPI 3.1 la `/api/v1/openapi.json` + `API.md`, panou UI Setari -> Acces API. Desktop: zero impact. Predecesor v2.39.0: fix major deploy web — sesiunea se stabileste automat la incarcare, se reinnoieste in fundal si se autorecupereaza la 401.)
**Versiune curenta reala:** v2.43.4
