...
**Data curenta:** 2026-05-05 (v2.13.0: export alerte + raport zilnic email)
**Versiune curenta reala:** v2.13.0
**Status global curent:** Sprint monitoring + web mode livrat pana la PR-11 si patch-uri UX v2.10.x → v2.13.x. v2.13.0 livreaza export alerte (Excel/PDF cu link portal.just.ro per dosar) in 3 moduri (`ids` / `filters` / `range`, cap 10k) plus raport zilnic email cu fereastra "ziua precedenta" controlat de field nou `dailyReportEnabled` in `owner_email_settings` (migration `0015`). PR-10 si PR-12 sunt eliminate prin decizia #11 din `EXECUTION-ROADMAP.md`; web cutover ramane reevaluabil separat.
