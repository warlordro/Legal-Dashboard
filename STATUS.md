...
**Data curenta:** 2026-05-12 (v2.20.8: Batch 2 + Batch 4 din FIXES-TODO.md — operator visibility (`source_partial` alert in spate de `MONITORING_PARTIAL_ALERTS_ENABLED=1`, `/health` `emailConfigured`, `preMigrationBackup` pe fatalBoot, cleanup `-wal`/`-shm` pe auto-revert, VACUUM splash) + scheduler/captcha reliability (`.catch` pe runOne, `AbortSignal.timeout(15s)` pe getBalance, race-mode sleep signal-aware, daily report retry [5/15/45]min, rate-limit periodic sweep 5min); zero schimbari schema/API, 854/854 backend + 100/100 frontend)
**Versiune curenta reala:** v2.20.8
