...
**Data curenta:** 2026-05-13 (v2.23.0: master switch monitoring per-owner. Migration 0020 `owner_monitoring_settings` cu partial index pe `monitoring_enabled = 0` pentru anti-join scheduler. Backend repository `isMonitoringEnabled` / `setMonitoringEnabled`, `claimDueJobs` filtrat via anti-join, rute `GET/PUT /api/v1/monitoring/master-switch` cu Zod `.strict()` (422 pe payload invalid) si audit `monitoring.master_switch.on/.off` cu `actor_id` + `request_id` propagate (scris doar la `changed=true`). Frontend hook `useMonitoringMasterSwitch` cu AbortController + optimistic flip + rollback + rethrow, buton in headerul cardului "Monitorizari active" (langa Excel/PDF). Cand master e off, iconita per-rand de pauza individuala (`job.active`) e fortata la `Play` ca semnal vizual. 926 backend pass + 102 frontend pass; tsc + biome verde.)
**Versiune curenta reala:** v2.23.0
