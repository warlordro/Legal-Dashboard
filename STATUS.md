...
**Data curenta:** 2026-06-02 (v2.36.2: fix functional in "Cautare Dosare" / "Cautare Termene" / monitoring pe nume — o cautare dupa un nume cu abreviere punctata (ex. `EURO ASFALT D.O.O. SARAJEVO`) returna zero rezultate desi dosarul exista pe PortalJust. Cauza: PortalJust indexeaza abrevierile fara puncte (`D.O.O.` indexat ca tokenul `DOO`), iar query-ul cu puncte e spart in litere izolate (D/O/O) care nu se potrivesc. Fix: `stripSearchDots` in `backend/src/soap.ts` elimina punctele din `numeParte` inainte de SOAP — singurul choke point, deci acopera dosare + termene + monitoring. Lossless, chirurgical pe `numeParte`. +5 teste backend, verificat empiric contra serviciului live.)
**Versiune curenta reala:** v2.36.2
