...
**Data curenta:** 2026-05-20 (v2.34.0: web hardening 4 P0 + 8 P1 din auditul intern. GET/device-code off, device_codes RL ip+UA, admin guards tenant-keys, SOAP retry budget + circuit breaker, captcha balance per-tenant TTL, owner-scoped tenant key guard, body-key warning hardening, per-user captcha quota cu UI dual-unit, CI fixtures off + RUNBOOK.md + offsite backup hook env-configurabil. Sentry SDK amanat la v2.35.0. Desktop ZERO impact.)
**Versiune curenta reala:** v2.34.0
