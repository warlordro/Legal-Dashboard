...
**Data curenta:** 2026-05-16 (v2.28.0: OpenRouter toggle in admin panel cu 2 niveluri - mode `native|openrouter` si stack `western|chinese` cand OpenRouter este activ. Native ramane backward-compatible 100%; OpenRouter are stack vestic mirror al modelelor native si stack chinezesc premium GLM 5.1/Kimi K2.6/Qwen 3.6 Max. Persistenta per owner prin `owner_ai_settings`, `ai_usage` accepta provider `openrouter` + `routing_tag`, multi-agent refuza mixarea stack-urilor cu `STACK_MIX_FORBIDDEN`, web mode accepta doar `OPENROUTER_API_KEY` din env, iar `OPENROUTER_DISABLED=1` opreste OpenRouter fara fallback silent.)
**Versiune curenta reala:** v2.28.0
