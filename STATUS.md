...
**Data curenta:** 2026-05-25 (v2.36.1: fix UI in "Cautare Dosare" — highlight-ul si filtrele client-side nu mai trateaza forma juridica (SC, SRL, SA, PFA, LLC, etc.) ca identitate. Cand cauti `SC ACME SRL`, doar `ACME` e colorat galben, iar filtrarea pe rol potriveste partile listate fara prefixul / sufixul juridic. Query-ul SOAP catre PortalJust ramane verbatim — zero schimbari pe wire. Helper partajat `frontend/src/lib/legalSuffix.ts` + 13 teste noi. Modificare strict frontend, fara schimbari pe backend / SOAP / DB.)
**Versiune curenta reala:** v2.36.1
