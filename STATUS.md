...
**Data curenta:** 2026-05-13 (v2.24.0: filtru text incremental peste rezultatele cautarii RNPM. Migration 0021 `idx_rnpm_avize_owner_search` + boot-time probe. Backend route `POST /api/rnpm/search/:searchId/filter` cu Zod validation, owner isolation, anti-enumeration 404, timeout 5s, truncare 1500 ID-uri si kill switch `RNPM_RESULTS_FILTER_DISABLED=1`; logurile pastreaza `qLen`, nu raw `q`. Repository helper dedicat cauta in 24 campuri normalizate fara sa atinga `getAvize()`. Frontend client + hook `useRnpmResultsFilter` cu debounce 300ms si AbortController, input live in `RnpmResultsTable`, bannere `missingDetails` / `truncated`, export + paginare pe setul filtrat. 51 teste noi pe migration/repository/EXPLAIN/cross-tenant/route/hook/component.)
**Versiune curenta reala:** v2.24.0
