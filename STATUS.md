...
**Data curenta:** 2026-05-14 (v2.27.2: fix UI — bulk-dismiss confirm din /alerte si popover-ul de instante din /monitorizare nu mai apar lipite de marginea stanga; tag-ul nativ `<dialog open>` inlocuit cu `<div>` + `role="alertdialog"`/`role="dialog"` + flex centering, acelasi pattern ca `<ConfirmProvider>`. Plus integrare interna F11-F1 OriginGuard hardening pe branch `feat/f11-1-originguard-hardening`: backend impune `X-Legal-Dashboard-Desktop` pe POST/DELETE admin body-less, frontend injecteaza header-ul global, `originGuard` returneaza envelope-shape la refuz. F11 ramane work-in-progress pentru web mode.)
**Versiune curenta reala:** v2.27.2
