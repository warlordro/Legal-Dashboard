...
**Data curenta:** 2026-05-15 (v2.27.4: release de consolidare. Inchide Faza 11 (F11-F2..F11-F5: shell:true scos din rebuild electron, separare auth/health, split frontend export per-domain, biome gate fara continue-on-error) si proiectul "biome total cleanup" (9 PR-uri Codex care duc biome la 0 errors si urca gate-ul permanent in CI). Absorbite trei runde CodeRabbit (#37 #38 #39) si trei polish-uri scurte: BarChart Tooltip cursor transparent + fara animatie, plafon export RNPM 500 -> 5000 ids (EXPORT_BODY_LIMIT 64KB -> 256KB), nitpicks CI permissions/concurrency pe lint-test.yml. Hard constraints respectate: SQL raw doar in backend/src/db/**, owner_id pe toate tabelele, DOMPurify activ, whitelist URL neatinsa.)
**Versiune curenta reala:** v2.27.4
