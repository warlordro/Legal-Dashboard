...
**Data curenta:** 2026-06-11 (v2.37.1: hardening post-full-review — monitoring alerts corecte (partial-failure carry-forward, dedup ancorat per-baseline, sticky multi-instanta, SOAP false-empty guard), RNPM timeout + TTL 15min, down-migratii cu cleanup _schema_versions + 0035 idx audit_log(ts), reziduuri ICCJ inchise, Caddy auth fix. Verificat: 1425+ backend + 232 frontend, biome/tsc/build curate. Predecesor v2.37.0: integrare ICCJ via live-proxy scj.ro — cautare dosare cu toggle de sursa, termene-pe-dosar, imbogatire server-side (categorie + rolul partilor + sedinte), metrici source-aware, monitoring `iccj` (migrarea 0034). Review dublu (10 agenti + Codex) + 10 fix-uri F1-F10: identitate monitoring pe `iccj_id`, conversie data DD.MM.YYYY, izolare timeout per-item + buget enrich, parser fail-loud la drift, dedup joburi, deep-link source-aware, kill-switch `ICCJ_ROUTES_DISABLED`. Verificat: 1374 teste backend + 232 frontend, smoke live.)
**Versiune curenta reala:** v2.37.1
