...
**Data curenta:** 2026-05-11 (v2.20.7: polish release — export "toate avizele filtrate" in Baza locala RNPM (client-side batching pe `/saved` + `/saved/export`); sheet "Debitori" -> "Parti" la export (entitati cu rol Cesionar/Cedent/Garant, nu doar debitori — TELECREDIT IFN 106× Cesionar verificat empiric); toggle in-memory in Setari pentru notificarile Windows/macOS la alerte (bulina + pagina Alerts neafectate, fara queue la reactivare); BulkSearch RNPM ramane montat la schimbare de tab; backend neatins, 844/844 + frontend 100/100)
**Versiune curenta reala:** v2.20.7
