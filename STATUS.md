...
**Data curenta:** 2026-06-25 (v2.39.0: fix major pentru deploy-ul web — SPA-ul nu apela niciodata bridge-ul `POST /api/v1/auth/oauth2/sync`, deci cookie-ul de sesiune nu se minteia si fiecare apel `/api` intorcea 401 "Token de autentificare necesar" desi login-ul Google reusea. Acum sesiunea se stabileste automat la incarcare (`useSessionBootstrap` + gate pe `AuthedApp`), se reinnoieste in fundal (`useSessionKeepAlive` ~50min + wake pe visibilitychange/online) si se autorecupereaza la 401 (interceptor deduplicat + retry in `apiFetch` via `ensureWebSession`), plus normalizarea guard-ului de auth-endpoint si largirea mesajelor de eroare. Desktop: zero impact. Predecesor v2.38.0: refresh modele AI (Opus 4.8, Gemini 3.5 Flash), eliminarea stack-ului OpenRouter chinezesc si hardening de securitate (revocare JWT, cookie SameSite Strict).)
**Versiune curenta reala:** v2.39.0
